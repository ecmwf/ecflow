...
BOOST_AUTO_TEST_CASE( test_event_and_query )
{
   ...
   BOOST_CHECK_MESSAGE(TestFixture::client().query("event",task_a->absNodePath(),"ecflow_1527")==0,"query command failed " << TestFixture::client().errorMsg());
   BOOST_CHECK_MESSAGE(TestFixture::client().get_string() == "clear","Expected query of event to return 'clear' but found " << TestFixture::client().get_string());

...
public:
   ...
   AutoArchiveAttr(int hour, int minute, bool relative ) : timeStruct_(hour, minute), relative_(relative), days_(false) {}
   AutoArchiveAttr(const TimeSlot& ts,   bool relative ) : timeStruct_(ts),           relative_(relative), days_(false) {}
   AutoArchiveAttr(int days) : timeStruct_( TimeSlot(days*24,0) ), relative_(true), days_(true) {}

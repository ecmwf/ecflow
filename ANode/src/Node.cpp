...
Node::Node(const Node& rhs)
    : parent_(nullptr), n_(rhs.n_), st_(rhs.st_), d_st_(rhs.d_st_), vars_(rhs.vars_),
      c_expr_((rhs.c_expr_) ? new Expression(*rhs.c_expr_) : nullptr),
      t_expr_((rhs.t_expr_) ? new Expression(*rhs.t_expr_) : nullptr), meters_(rhs.meters_), events_(rhs.events_),
      labels_(rhs.labels_), times_(rhs.times_), todays_(rhs.todays_), crons_(rhs.crons_), dates_(rhs.dates_),
      days_(rhs.days_), late_((rhs.late_) ? new ecf::LateAttr(*rhs.late_) : nullptr),
      misc_attrs_((rhs.misc_attrs_) ? new MiscAttrs(*rhs.misc_attrs_) : nullptr), repeat_(rhs.repeat_),
      inLimitMgr_(rhs.inLimitMgr_), flag_(rhs.flag_),

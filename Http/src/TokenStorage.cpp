...
string hmac_sha256(const string& salt, const std::string& token) {
    ...
    HMAC (EVP_sha256(), salt.c_str(), salt.size(), reinterpret_cast<const unsigned char*> (token.c_str()), token.size(), hash, &hash_len);
    std::stringstream ss;
...
string hmac_pbkdf2_sha256(const string& salt, const std::string& token, int num_iterations) {
    ...
    PKCS5_PBKDF2_HMAC (token.c_str(), token.size(), reinterpret_cast<const unsigned char*> (salt.c_str()), salt.size(), num_iterations, EVP_sha256(), SHA256_DIGEST_LENGTH, hash);
    std::stringstream ss;
...
bool is_supported_hmac(const std::string& method) {
    if (method == "sha256" || method.find("pbkdf2:sha256") != string::npos) return true;
    return false;
}
...
bool TokenStorage::verify(const std::string& token) const {
   ...
   for (const auto& t : tokens_) {
       ...
       if (hashed == t.hash &&
           (t.expires.time_since_epoch().count() == 0 || t.expires > now) && 
           (t.revoked.time_since_epoch().count() == 0 || t.revoked > now)
       ) {
           if (opts.verbose) printf("Token for '%s' authenticated succesfully\n", t.description.c_str());
           return true;
       }
       // printf("%s %s %s to %s should be %s\n", t.method.c_str(), t.salt.c_str(), token.c_str(), hashed.c_str(), t.hash.c_str());
   }
   return false;
}
...
std::vector<Token> ReadTokens(const std::string& filename) {
   ...
   for (const auto& o : j) {
       try {
          ...
          new_tokens.emplace_back(elems[2],
                                  elems[1],
                                  elems[0],
                                  o.at("description").get<string>(),
                                  expires_at,
                                  revoked_at
          );
       }
   ...
   if (opts.verbose) printf("Read %ld tokens\n", new_tokens.size());

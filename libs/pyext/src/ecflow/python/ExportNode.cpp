...
///
/// @brief Add a RepeatDateTimeList attribute to the node.
///
/// @param self The node.
/// @param d The repeat-datetime-list attribute to add.
/// @return The node (for method chaining).
///
node_ptr Node_add_repeat_datetime_list(node_ptr self, const RepeatDateTimeList& d) {
    self->addRepeat(d);
    return self;
}
...
void export_Node(py::module& m) {
    ...
    py::class_<Node, std::shared_ptr<Node>>(m, "Node", py::dynamic_attr(), DefsDoc::node_doc())
        ...
        .def("add_repeat", &Node_add_repeat_datetime_list, DefsDoc::add_repeat_datetime_list_doc())

...
void pyutil_list_to_str_vec(const bp::list& list, std::vector<std::string>& int_vec);
void pyutil_list_to_str_vec(const bp::list& list, std::vector<Variable>& vec);
void pyutil_dict_to_str_vec(const bp::dict& dict, std::vector<std::pair<std::string, std::string>>& str_pair);

...
const char* ClientDoc::query() {
    return "Query the status of a node attribute, or evaluate a trigger expression, without blocking. The result is\n"
           "returned as a string.\n\n"
           "- state,     returns :code:`unknown` | :code:`complete` | :code:`queued` | :code:`aborted` | "
           ":code:`submitted` | :code:`active`\n"
           "- dstate,    returns :code:`unknown` | :code:`complete` | :code:`queued` | :code:`suspended` | "
           ":code:`aborted` | :code:`submitted` | :code:`active`\n"
           ...
           "                              # the path '/', the server itself, can only be used with 'state' or "
           "'variable'\n"
           ...
           ".. code-block:: python\n\n"
           "   try:\n"
           "       ci = Client()                                                  # use default host(ECF_HOST) & "
           "port(ECF_PORT)\n"
           "       res = ci.query('state','/path/to/node')                        # returns node state as a string\n"
           "       res = ci.query('dstate','/path/to/node')                       # returns node state, can include "
           "suspended\n"
           "       res = ci.query('repeat','/path/to/node')                       # returns current repeat value as a "
           "string\n"
           "       res = ci.query('repeat','/path/to/node','next')                # returns next repeat value as a "
           "string\n"
           ...
           "       res = ci.query('limit_max','/path/to/node','limit_name')       # returns max limit value as a "
           "string\n"
           "       res = ci.query('label','/path/to/node','label_name')           # returns label value as a string\n"
           "       res = ci.query('variable','/path/to/node','var')               # returns variable value as a "
           "string\n"
           "       res = ci.query('variable','/path/to/node','var',evaluate=True) # returns variable value, references "
           "resolved\n"
           "       res = ci.query('variable','/','SCHOST')                        # returns value of server variable "
           "as a string\n"
           "       res = ci.query('trigger','/path/to/node','/joe90 == complete') # returns 'true' | 'false' as a "
           "string\n"
...
const char* ClientDoc::alter() {
    return "Alter command is used to change the attributes of a node\n"
           ...
           "   # if no name specified the event is set, otherwise name must be 'set' or "
           "'clear'\n"
           "\n"
           " change meter name value\n"
           "   # The meter value must be convertible to an integer, and between meter "
           "min-max range.\n"
           "\n"

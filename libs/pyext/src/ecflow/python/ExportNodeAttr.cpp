...
static RepeatDateTimeList create_RepeatDateTimeList(const std::string& name, const py::list& list) {
    auto n = len(list);
    std::vector<ecf::Instant> vec;
    vec.reserve(n);
    for (size_t i = 0; i < n; i++) {
        auto extracted = list[i].cast<std::string>();
        try {
    ...
    return RepeatDateTimeList(name, vec);
}
...
static std::string str_print_aviso(const ecf::AvisoAttr& attr) {
    return ecf::to_python_string(attr);
}

static std::string str_print_mirror(const ecf::MirrorAttr& attr) {
    return ecf::to_python_string(attr);
}
...
void export_NodeAttr(py::module& m) {
    ...
    py::class_<RepeatDateTimeList>(m, "RepeatDateTimeList", NodeAttrDoc::repeat_datetimelist_doc())
        .def(py::init(&create_RepeatDateTimeList))
        .def(py::self == py::self)                               // __eq__
    ...
    py::class_<ecf::AvisoAttr>(m, "AvisoAttr", NodeAttrDoc::aviso_doc())
        ...
        .def(py::self == py::self)                           // __eq__
        .def("__str__", &str_print_aviso)                    // __str__
    ...
    py::class_<ecf::MirrorAttr>(m, "MirrorAttr", NodeAttrDoc::mirror_doc())
        ...
        .def(py::self == py::self)                            // __eq__
        .def("__str__", &str_print_mirror)                    // __str__

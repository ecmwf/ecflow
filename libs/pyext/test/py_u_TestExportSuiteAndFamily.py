...
class TestFamilyVec:
    ...
    def test_getitem_returns_correct_family(self):
        ...
        assert fv[0].name() == "alpha"
        assert fv[1].name() == "beta"
...
class TestSuiteVec:
    ...
    def test_getitem_returns_correct_suite(self):
        ...
        assert sv[0].name() == "alpha"
        assert sv[1].name() == "beta"
...
class TestNodeContainer:
    ...
    def test_add_family_by_name_returns_family(self):
        ...
        assert isinstance(ret, ecf.Family)
        assert ret.name() == "child_fam"
    ...
    def test_add_task_by_name_returns_task(self):
        ...
        assert isinstance(ret, ecf.Task)
        assert ret.name() == "my_task"
    ...
    # ------------------------------------------------------------------
    # py::keep_alive — family/task Python wrappers survive after add
    # ------------------------------------------------------------------

    def test_add_family_by_ptr_child_survives_original_del(self):
        """The C++ Family added via add_family(ptr) is still accessible after the original Python
        reference is deleted and GC runs."""
        s = ecf.Suite("s")
        f = ecf.Family("kept")
        s.add_family(f)
        del f
        gc.collect()
        found = s.find_family("kept")
        assert found is not None
        assert found.name() == "kept"

    def test_add_task_by_ptr_child_survives_original_del(self):
        """The C++ Task added via add_task(ptr) is still accessible after the original Python
        reference is deleted and GC runs."""
        s = ecf.Suite("s")
        t = ecf.Task("kept")
        s.add_task(t)
    ...
    def test_add_family_dynamic_attr_survives(self):
        ...
        assert found._marker == "hello"

    def test_add_task_dynamic_attr_survives(self):
        """A dynamic attribute set on a Task before add_task(ptr) is still present on the
        Python wrapper returned by find_task (py::keep_alive preserves the wrapper)."""
        s = ecf.Suite("s")
        t = ecf.Task("tagged")
        t._marker = "world"
        s.add_task(t)
...
class TestSuite:
    ...
    def test_create_with_children_sets_variables(self):
        ...
        assert s.find_variable("MY_VAR").value() == "world"
    ...
    def test_get_clock_returns_clock_after_add(self):
        ...
        assert isinstance(result, ecf.Clock)
    ...
    def test_get_end_clock_returns_none_when_no_end_clock_set(self):
        ...
        s = ecf.Suite("s")
        assert s.get_end_clock() is None

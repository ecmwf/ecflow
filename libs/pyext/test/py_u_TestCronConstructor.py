...
def test_create_cron_that_runs_every_day_at_2pm():
    cron = ecf.Cron("14:00")
    assert cron.time() == ecf.TimeSeries(14, 00)


def test_create_cron_that_runs_every_30_minutes_between_0am_and_8pm_on_the_first_six_days_of_the_months_from_january_until_july():
    cron = ecf.Cron(
        "+00:00 20:00 00:30",
    ...
    assert cron.time() == ecf.TimeSeries(
        ecf.TimeSlot(0, 0), ecf.TimeSlot(20, 0), ecf.TimeSlot(0, 30), True
    )
    assert cron.days_of_month == [1, 2, 3, 4, 5, 6]
    assert cron.months == [1, 2, 3, 4, 5, 6]


def test_create_cron_relative_to_suite_start_time_or_task_requeue():
    cron = ecf.Cron(
        "+00:15 23:00 00:30",
        days_of_week=[0, 1, 2],
        days_of_month=[4, 5, 6],
    ...
    assert cron.time() == ecf.TimeSeries(
        ecf.TimeSlot(0, 15), ecf.TimeSlot(23, 0), ecf.TimeSlot(0, 30), True
    )
...
def test_create_cron_based_on_time_series():
...
def test_create_cron_with_defaults_and_set_time_series():
    cron = ecf.Cron()
...
def test_create_cron_that_runs_on_the_last_day_of_the_month():
    cron = ecf.Cron()
    cron.set_last_day_of_the_month()

    assert cron.last_day_of_the_month() is True


def test_create_cron_that_runs_on_the_last_weekdays_of_the_month():
    cron = ecf.Cron()

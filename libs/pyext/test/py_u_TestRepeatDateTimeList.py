...
def test_construction():
    ...
    rep = ecflow.RepeatDateTimeList(
        "DT", ["20240101T000000", "20240102T000000", "20240103T000000"]
    )
    assert rep.name() == "DT", f"name should be 'DT' but got '{rep.name()}'"
    assert rep.start() is not None, "start() should return a value"
    assert (
        rep.start() == 1704067200
    ), "Expected 1704067200 (i.e. second since 19700101T000000 until 20240101T000000)"
    assert rep.end() is not None, "end() should return a value"
    assert (
        rep.end() == 1704240000
    ), "Expected 1704240000 (i.e. second since 19700101T000000 until 20240103T000000)"
...
def test_inequality():
    ...
    assert (
        r1 != r2
    ), "RepeatDateTimeList objects with different lists should not compare equal"
...
def test_str_round_trip():
    ...
    s = str(rep)
    assert (
        "repeat datetimelist" in s
    ), f"str should contain 'repeat datetimelist', got: {s}"
...
def test_add_to_task_via_constructor():
    ...
    t = ecflow.Task("t") + ecflow.RepeatDateTimeList(
        "DT", ["20240101T000000", "20240102T000000"]
    )
    repeat = t.get_repeat()
...
def test_add_repeat_method():
    ...
    t = ecflow.Task("t")
    t.add_repeat(
        ecflow.RepeatDateTimeList("DT", ["20240101T000000", "20240102T000000"])
    )
    repeat = t.get_repeat()
...
def test_trigger_arithmetic():
    ...
    t1 = s.add_task("t1")
    t1.add_repeat(
        ecflow.RepeatDateTimeList("DT", ["20090101T000000", "20091231T000000"])
    )
    t2 = s.add_task("t2")
    ...
    ), "Trigger should not fire: 20090101T000000 < 20100601T000000"
    ...
    # After moving to second element (20090131T000000):
    # 20090131T000000 + 86400 == 20090201T000000
    t1.delete_repeat()
    t1.add_repeat(
...
def test_trigger_datetime_instant_rhs_arithmetic():
    ...
    assert (
        len(defs.check()) == 0
    ), "Expected no errors: 't1:DT ge 19700102T000000 + 86400'"
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to fire: DT(20090101T000000) >= 19700102T000000 + 86400"
    ...
    assert (
        len(defs.check()) == 0
    ), "Expected no errors: 't1:DT ge (20240701T000000 + 60)'"
    assert (
        not t2.evaluate_trigger()
    ...
    assert (
        len(defs.check()) == 0
    ), "Expected no errors: 't1:DT ge (20090101T000000 - 1)'"
    assert (
        t2.evaluate_trigger()
...
def test_defs_str_contains_datetimelist():
    ...
    defs_str = str(defs)
    assert (
        "repeat datetimelist" in defs_str

...
class Test_dunder_rshift(unittest.TestCase):
    def test_node_dunder_rshift(self):
        ...
        self.assertEqual(
            len(list(suite)),
            4,
            "expected 4 children but found " + str(len(list(suite))),
        )

        self.assertEqual(
            str(suite.t2.get_trigger()),
            "t1 == complete",
            "Trigger not as expected: " + str(suite.t2.get_trigger()),
        )
        self.assertEqual(
            str(suite.t3.get_trigger()),
            "t2 == complete",
            "Trigger not as expected: " + str(suite.t3.get_trigger()),
        )
        self.assertEqual(
            str(suite.t4.get_trigger()),
            "t3 == complete",
        ...
        self.assertEqual(
            len(list(fam)), 4, "expected 4 children but found " + str(len(list(fam)))
        )

        self.assertEqual(
            str(fam.t2.get_trigger()),
            "t1 == complete",
            "Trigger not as expected: " + str(fam.t2.get_trigger()),
        )
        self.assertEqual(
            str(fam.t3.get_trigger()),
            "t2 == complete",
            "Trigger not as expected: " + str(fam.t3.get_trigger()),
        )
        self.assertEqual(
            str(fam.t4.get_trigger()),
            "t3 == complete",
...
class Test_dunder_add(unittest.TestCase):
    ...
    def test_node_dunder_add_meter(self):
        ...
        self.assertEqual(
            len(suite), 3, "expected 3 children but found " + str(len(suite))
        )
        self.assertEqual(
            len(list(suite.t3.events)),
            1,
            "expected 1 event found " + str(len(list(suite.t3.events))),
        )
        self.assertEqual(
            len(list(suite.t3.meters)),
            1,
            "expected 1 event found " + str(len(list(suite.t3.meters))),
        )
        self.assertEqual(
            len(list(suite.t3.labels)),
            1,
            "expected 1 event found " + str(len(list(suite.t3.labels))),
        )
    ...
    def test_node_dunder_add(self):
        ...
        self.assertEqual(
            len(suite), 1, "expected 1 children but found " + str(len(suite))
        )
        self.assertEqual(
            len(list(suite.t3.events)),
            1,
        ...
        self.assertEqual(
            len(suite), 4, "expected 4 children but found " + str(len(suite))
        )
    ...
    def test_dunder_add_all(self):
        ...
        self.assertEqual(
            len(defs.s1), 1, "Expected 1 nodes but found " + str(len(defs.s1))
        )
        self.assertEqual(
            len(list(t1.variables)),
            8,
            "expected 8 variables but found " + str(len(list(t1.variables))),
        )
...
class TestListComprehension(unittest.TestCase):
    def test_suite_list(self):
        ...
        self.assertEqual(
            len(defs), 10, " expected 10 suites but found " + str(len(defs))
        )
    ...
    def test_family_list(self):
        ...
        self.assertEqual(
            len(defs.suite), 5, " expected 5 familes but found " + str(len(defs.suite))
        )

        defs = Defs([Suite("xx", [Family("f{0}".format(i)) for i in range(1, 6)])])
        self.assertEqual(
            len(defs.xx), 5, " expected 5 familes but found " + str(len(defs.xx))
        )
    ...
    def test_task_list(self):
        ...
        self.assertEqual(
            len(defs.suite.f), 5, " expected 5 task but found " + str(len(defs.suite.f))
        )
    ...
    def test_task_list2(self):
        ...
        self.assertEqual(
            len(defs.suite), 13, " expected 13 nodes but found " + str(len(defs.suite))
        )
        self.assertEqual(
            len(defs.suite.f),
            5,
            " expected 5 nodes but found " + str(len(defs.suite.f)),
        )
        self.assertEqual(
            len(list(defs.suite.variables)),
            1,
            " expected 1 variable " + str(len(list(defs.suite.variables))),
        )
    ...
    def test_5Suite_with_5families_with_5tasks(self):
        ...
        for suite in defs:
            self.assertEqual(
                len(suite), 5, " expected 5 familes but found " + str(len(suite))
            )
            for fam in suite:
                self.assertEqual(
                    len(fam), 5, " expected 5 tasks but found " + str(len(fam))
                )
...
class TestTrigger(unittest.TestCase):
    ...
    def test_trigger_string_list(self):
        ...
        self.assertEqual(
            str(t),
            "a == complete AND b == complete AND c == complete",
    ...
    def test_trigger_node_list(self):
        ...
        t = Trigger([task])
        self.assertEqual(
            str(t), "/s/f/t == complete", "Trigger not as expected: " + str(t)
        )
    ...
    def test_add(self):
        ...
        self.assertEqual(
            str(expr), "1==1", "Trigger not as expected: " + expr.get_expression()
        )

        task.add(Trigger("2==1"))
        self.assertEqual(
            str(expr), "1==1 AND 2==1", "Trigger not as expected: " + str(expr)
        )
...
class TestComplete(unittest.TestCase):
    ...
    def test_Complete_node_list(self):
        ...
        t = Complete([task])
        self.assertEqual(
            str(t), "/s/f/t == complete", "Complete not as expected: " + str(t)
        )
    ...
    def test_add(self):
        ...
        self.assertEqual(
            str(expr), "1==1", "Complete not as expected: " + expr.get_expression()
        )

        task.add(Complete("2==1"))
        self.assertEqual(
            str(expr), "1==1 AND 2==1", "Complete not as expected: " + str(expr)
        )
...
class TestDefsAdd(unittest.TestCase):
    ...
    def test_add_variables(self):
        ...
        self.assertEqual(
            len(list(defs.user_variables)), 3, "Expected 3 variables to be added"
        )
...
class TestEdit(unittest.TestCase):
    ...
    def test_edit(self):
        ...
        self.assertEqual(
            ...
            8,
            "expected 8 variables but found " + str(len(list(t.variables))),
        )
...
class TestAddAll(unittest.TestCase):
    def test_add(self):
        ...
        self.assertTrue(t1 is not None, "Can't find t1")
        self.assertEqual(
            len(defs.s1), 6, "Expected 6 nodes but found " + str(len(defs.s1))
        )
...
class TestComparison(unittest.TestCase):
    ...
    def test_compare_with_add(self):
        ...
        self.assertEqual(
            self.defs1, defs, "defs not equal\n" + str(self.defs1) + "\n\n" + str(defs)
        )

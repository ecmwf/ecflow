...
def test_create_flag_from_default_parameters():
    flag = ecf.Flag()
...
def test_set_clear_reset_flag():
    ...
    for flag_value in FLAG_VALUES:
        ...
        assert flag.is_set(
            flag_value
        ), "expected flag %r to be set" % flag.type_to_string(flag_value)
        # Clear the flag
        flag.clear(flag_value)
        assert not flag.is_set(flag_value), "expected flag {0} not to be set".format(
            flag.type_to_string(flag_value)
        )
    ...
    assert (
        str(flag)
        == "force_aborted,user_edit,task_aborted,edit_failed,ecfcmd_failed,killcmd_failed,statuscmd_failed"
...
def test_access_empty_flag():
    suite = ecf.Suite("s1")
    flag = suite.get_flag()  # the flag should be empty for a newly created node.
    for flag_value in FLAG_VALUES:
        assert not flag.is_set(
            flag_value
        ), "expected flag %r not to be set" % flag.type_to_string(flag_value)
    assert str(flag) == "", "expected empty string, for an empty flag"

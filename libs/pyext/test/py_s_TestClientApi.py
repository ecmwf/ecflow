...
def test_client_get_server_defs(ci, protocol):
    ...
    ci.get_server_defs()
    assert (
        ci.get_defs().find_suite("s1") is not None
...
def test_client_restart_server(ci):
    ...
    sync_local(ci)
    assert (
        ci.get_defs().get_server_state() == SState.RUNNING
...
def test_client_halt_server(ci):
    ...
    sync_local(ci)
    assert (
        ci.get_defs().get_server_state() == SState.HALTED
...
def test_client_shutdown_server(ci):
    ...
    sync_local(ci)
    assert (
        ci.get_defs().get_server_state() == SState.SHUTDOWN
...
def test_client_requeue_with_multiple_paths(ci, protocol):
    ...
    sync_local(ci)
    task1 = ci.get_defs().find_abs_node(
        "/test_client_requeue_with_multiple_paths/f1/t1"
    )
    task2 = ci.get_defs().find_abs_node(
...
def test_client_free_dep(ci, protocol):
    ...
    while 1:
        ...
        if t1.get_state() == State.queued:
            ci.free_time_dep(t1_path)
        if t2.get_state() == State.queued:
            ci.free_date_dep(t2_path)
        if t3.get_state() == State.queued:
            ci.free_trigger_dep(t3_path)
        if t4.get_state() == State.queued:
            ci.free_all_dep(t4_path)
...
def test_client_ch_suites(ci):
    ...
    sync_local(ci)
    assert (
        len(list(ci.get_defs().suites)) == 6
    ), "Expected 6 after load, but found " + str(len(list((ci.get_defs().suites))))
...
def test_client_ch_drop(ci):
    ...
    sync_local(ci)
    assert len(list(ci.get_defs().suites)) == 6, "Expected 6 suites but found " + str(
        len(list((ci.get_defs().suites)))
    )
...
def test_client_ch_drop_user(ci):
    ...
    try:
        ...
        assert (
            len(list(ci.get_defs().suites)) == 3
        ), "Expected 3 suites but found " + str(len(list((ci.get_defs().suites))))
...
def test_client_ch_add(ci):
    ...
    try:
        ...
        sync_local(ci)
        assert (
            len(list(ci.get_defs().suites)) == 2
        ), "Expected 2 suites but found " + str(len(list((ci.get_defs().suites))))

        suite_names = ["s3", "s4"]
        ci.ch_add(ci.ch_handle(), suite_names)  # add suites s3,s4 using last handle
        sync_local(ci)
        assert (
            len(list(ci.get_defs().suites)) == 4
        ), "Expected 4 suites but found " + str(len(list((ci.get_defs().suites))))
...
def test_client_ch_remove(ci):
    ...
    try:
        ...
        suite_names = ["s2"]
        ci.ch_remove(
            ci.ch_handle(), suite_names
        )  # remove suites s2 from the last added handle\n"
        sync_local(ci)
        assert (
            len(list(ci.get_defs().suites)) == 1
        ), "Expected 1 suites but found " + str(len(list((ci.get_defs().suites))))
...
def test_client_alter_change(ci, protocol):
    ...
    assert variable.value() == "changed_var", (
        "Expected alter of variable to be 'change_var' but found " + variable.value()
    )
    ...
    assert variable.value() == "--fred --bill", (
        "Expected alter of variable to be '--fred --bill' but found " + variable.value()
    )
    ...
    assert event.value() == True, "Expected alter of event to be set but found " + str(
        event.value()
    )
    ...
    task_t1 = ci.get_defs().find_abs_node(t1)
    trigger = task_t1.get_trigger()
    assert trigger.get_expression() == "/test_client_alter_change/f1/t2 == complete", (
    ...
    task_t1 = ci.get_defs().find_abs_node(t1)
    complete = task_t1.get_complete()
    assert complete.get_expression() == "/test_client_alter_change/f1/t2 == active", (
    ...
    assert limit.limit() == 2, "Expected alter of limit_max to be 2 but found " + str(
        limit.limit()
    )
    ...
    limit = task_t1.find_limit("limit")
    assert limit is not None, "Expected to find limit"
    assert limit.value() == 2, "Expected alter of limit_value to be 2 but found " + str(
        limit.value()
    )
...
def test_client_replace(ci, on_disk, protocol):
    ...
    assert (
        ci.get_defs().find_abs_node("/s1/f2/t1") is not None
    ...
    assert (
        ci.get_defs().find_abs_node("/s1/f1/t1") is not None
    ...
    assert (
        ci.get_defs().find_abs_node("/s1/f1/t1") is None
    ), "Expected NOT to find task /s1/f1/t1\n" + str(ci.get_defs())
    assert (
        ci.get_defs().find_abs_node("/s1/f2/t1") is None
    ...
    assert (
        ci.get_defs().find_abs_node("/s2/t1") is not None
...
def test_node_replace(ci):
    ...
    assert (
        ci.get_defs().s1.get_dstate() == DState.suspended
    ), "Expected node to be suspended:\n" + str(ci.get_defs())
...
def test_client_suspend(ci, protocol):
    ...
    assert ci.query("dstate", suite.get_abs_node_path()) == "suspended", (
        "Expected to find suite dstate suspended but found: " + res
    )
    assert ci.query("state", suite.get_abs_node_path()) == "queued", (
...
def test_client_delete_node_multiple_paths(ci, protocol):
    ...
    ci.delete_all()
    defs = create_defs(
        "test_client_delete_node_multiple_paths", ci.get_port(), protocol
    )
...
def test_ECFLOW_199(ci, protocol):
    ...
    # print(ci.get_defs())
    assert (
        len(list(ci.changed_node_paths)) == 0
    ), "Expected first call to sync_local, to have no changed paths but found " + str(

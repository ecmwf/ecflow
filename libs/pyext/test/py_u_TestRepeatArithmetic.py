...
def _test_repeat_arithmetic(repeat_to_add, repeat_to_add2):
    defs = ecflow.Defs()
    ...
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to evaluate. 20090101 - 1  == 20081231"

    # check addition
    t2.change_trigger("t1:YMD + 1 eq 20090102")
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to evaluate. 20090101 + 1  == 20090102"
    ...
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to evaluate. 20090131 + 1  == 20090201"


def _test_repeat_datetime_arithmetic(repeat_to_add, repeat_to_add2):
    defs = ecflow.Defs()
    s1 = defs.add_suite("s1")
    t1 = s1.add_task("t1").add_repeat(repeat_to_add)
    ...
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to evaluate. 20090101T000000 - 86400  == 20081231T000000"

    # check addition
    t2.change_trigger("t1:DT + 86400 eq 20090102T000000")
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to evaluate. 20090101T000000 + 86400  == 20090102T000000"
    ...
    assert (
        t2.evaluate_trigger()
    ), "Expected trigger to evaluate. 20090131 + 86400  == 20090201T000000"


def test_repeat_date_arithmetic():
    _test_repeat_arithmetic(
        ecflow.RepeatDate("YMD", 20090101, 20091231, 1),
        ecflow.RepeatDate("YMD", 20090131, 20101231, 1),
    )


def test_repeat_date_list_arithmetic():
    _test_repeat_arithmetic(
        ecflow.RepeatDateList("YMD", [20090101, 20091231]),
        ecflow.RepeatDateList("YMD", [20090131, 20101231]),
    )


def test_repeat_datetime_arithmetic():
    _test_repeat_datetime_arithmetic(
        ecflow.RepeatDateTime("DT", "20090101T000000", "20091231T000000", "24:00:00"),
        ecflow.RepeatDateTime("DT", "20090131T000000", "20101231T000000", "24:00:00"),
    )


def test_repeat_datetime_list_arithmetic():
    _test_repeat_datetime_arithmetic(

...
class TestDefs:
    ...
    def test_ctor_from_file_loads_suites(self):
        ...
        try:
            d = ecf.Defs(fname)
            assert "loaded" in [s.name() for s in d.suites]
        finally:
            os.unlink(fname)
    ...
    def test_add_extern_stored(self):
        ...
        exts = list(d.externs)
        assert "/suite/task:event" in exts
    ...
    def test_server_variables_nonempty(self):
        ...
        assert len(list(ecf.Defs().server_variables)) > 0
    ...
    def test_find_abs_node_not_found_returns_none(self):
        ...
        d = ecf.Defs()
        d.add_suite("s1")
        assert d.find_abs_node("/s1/nonexistent") is None
    ...
    def test_find_node_path_task(self):
        ...
        f = s.add_family("f1")
        f.add_task("t1")
        assert d.find_node_path("task", "t1") == "/s1/f1/t1"
    ...
    def test_get_all_nodes_empty_defs(self):
        ...
        assert d.get_all_nodes() if False else list(ecf.Defs().get_all_nodes()) == []
    ...
    def test_has_time_dependencies_false_when_none(self):
        ...
        assert not self._simple_defs().has_time_dependencies()
    ...
    def test_has_time_dependencies_true_with_time(self):
        ...
        t = s.add_task("t1")
        t.add_time("08:00")
        assert d.has_time_dependencies()
    ...
    def test_check_job_creation_returns_string(self):
        ...
        d = self._simple_defs()
        assert isinstance(d.check_job_creation(), str)
    ...
    def test_save_as_defs_roundtrip(self):
        ...
        try:
            d.save_as_defs(fname)
            d2 = ecf.Defs(fname)
            assert [s.name() for s in d2.suites] == ["s1"]
        finally:
            os.unlink(fname)
    ...
    def test_restore_from_checkpt_recovers_suites(self):
        ...
        try:
            ...
            assert "s1" in [s.name() for s in d2.suites]
        finally:
            os.unlink(cname)
    ...
    def test_add_returns_defs(self):
        ...
        assert isinstance(result, ecf.Defs)
    ...
    def test_str_user_variables_not_in_output(self):
        ...
        assert "MY_VAR" not in str(d)
        assert "MY_VAR" in [v.name() for v in d.user_variables]
    ...
    def test_iter_yields_suites(self):
        ...
        d.add_suite("beta")
        items = list(d)
        assert all(isinstance(s, ecf.Suite) for s in items)
    ...
    def test_is_hashable(self):
        """Defs is hashable."""
        assert isinstance(hash(ecf.Defs()), int)
    ...
    def test_copy_preserves_suites(self):
        ...
        d.add_suite("s1")
        c = copy.copy(d)
        assert "s1" in [s.name() for s in c.suites]

...
BOOST_AUTO_TEST_CASE(test_log_and_checkpt_write_errors) {
    ...
    if (!ClientEnvironment::hostSpecified().empty()) {
        std::cout << "Ignoring test when server is already running" << std::endl;
        return;
    }
    ...
    if (get_login_name() == "root") {
        std::cout << "Ignoring test when user is root." << std::endl;
        return;
    }

    bool debug_me = false;
    if (debug_me) {
        std::cout << "->Create a directory from where we will start the server\n";
    }
    std::string ecf_home = "test_log_and_checkpt_write_errors" + Pid::getpid();
    fs::create_directory(ecf_home);

    if (debug_me) {
        std::cout << "->change directory, before server start\n";
    }
...

    {
        ...
        if (debug_me) {
            std::cout << "->load a defs file to the server\n";
        }
        ...
        if (debug_me) {
            ...
            std::cout << ecf::as_string(*theClient.defs(), PrintStyle::STATE) << "\n";
        }

        if (debug_me) {
            std::cout << "->flush the log file, this will close the log file in the server\n";
        }
        BOOST_CHECK_MESSAGE(theClient.flushLog() == 0, "flushLog failed \n" << theClient.errorMsg());

        if (debug_me) {
            std::cout << "->remove write permission for " << ecf_home << "\n";
        }
        ...
        if (debug_me) {
            std::cout
                << "->write the checkpoint file, should fail, this should also try to re-open the log file and fail\n";
        }
        ...
        if (debug_me) {
            std::cout << "->get the defs from server, Check flags, check server variables\n";
        }
        BOOST_CHECK_MESSAGE(theClient.sync_local() == 0, "sync_local failed \n" << theClient.errorMsg());
        ...
        BOOST_CHECK_MESSAGE(theClient.defs()->server_state().variable_exists("ECF_CHECKPT_ERROR"),
                            "Expected to find ECF_CHECKPT_ERROR as a server variable");
        BOOST_CHECK_MESSAGE(theClient.defs()->server_state().variable_exists("ECF_LOG_ERROR"),
        ...
        if (debug_me) {
            std::cout << "->add write permission to " << ecf_home << "\n";
        }
        BOOST_CHECK_MESSAGE(chmod(ecf_home.c_str(), strtol("0755", nullptr, 8)) == 0,
                            "Can't chmod : " << strerror(errno));

        if (debug_me) {
            std::cout << "->flush log file again\n";
        }
        BOOST_CHECK_MESSAGE(theClient.flushLog() == 0, "flushLog failed \n" << theClient.errorMsg());

        if (debug_me) {
            std::cout << "->clear the flags -> this should *ALSO* delete the server user variables\n";
        }
        ...
        if (debug_me) {
            std::cout << "->Get the defs, Check flags are cleared and server variables are deleted\n";
        }
        BOOST_CHECK_MESSAGE(theClient.sync_local() == 0, "sync_local failed \n" << theClient.errorMsg());
        BOOST_CHECK_MESSAGE(!theClient.defs()->get_flag().is_set(Flag::CHECKPT_ERROR),
    ...
    if (debug_me) {
        std::cout << "->remove created directory " << ecf_home << "\n";
    }
    if (debug_me) {
        std::cout << "->current path = " << fs::current_path() << "\n";
    }

...
BOOST_AUTO_TEST_CASE(test_node_tree_lifecycle) {
    ...
    // Create a request to initialise Node: suite1/family1/a
    {
        TestHelper::invokeRequest(
            &defs,
            Cmd_ptr(new InitCmd(
                suite1_family1_a, Submittable::DUMMY_JOBS_PASSWORD(), Submittable::DUMMY_PROCESS_OR_REMOTE_ID(), 1)));
    ...
    // This should force suite1_family2_bb immediately into submitted/active state
    {
        ...
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new EventCmd(suite1_family1_a,
        ...
        BOOST_CHECK_MESSAGE(node->state() == NState::ACTIVE,
                            "State should be NState::ACTIVE but found to be " << NState::toString(node->state()));
        BOOST_CHECK_MESSAGE(suite->state() == NState::ACTIVE,
                            "Suite should be in NState::ACTIVE but found to be " << NState::toString(suite->state()));

        node_ptr nodebb = defs.findAbsNode(suite1_family2_bb);
        BOOST_CHECK_MESSAGE(nodebb->state() == NState::ACTIVE,
                            "State should be NState::ACTIVE but found to be " << NState::toString(nodebb->state()));
    }
    ...
    // This should immediately change suite1_family2_aa into submitted state
    {
        ...
        int meterValue        = 100;
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new MeterCmd(suite1_family1_a,
                                                       Submittable::DUMMY_JOBS_PASSWORD(),
                                                       Submittable::DUMMY_PROCESS_OR_REMOTE_ID(),
                                                       1,
                                                       metername,
        ...
        node_ptr node = defs.findAbsNode(suite1_family1_a);
        BOOST_CHECK_MESSAGE(node->state() == NState::ACTIVE,
                            "Expected Node '" << node->absNodePath() << "' to be NState::ACTIVE, but found "
        ...
        std::string errorMsg;
        BOOST_REQUIRE_MESSAGE(defs.checkInvariants(errorMsg), errorMsg);
    ...
    {
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new CompleteCmd(suite1_family1_a, Submittable::DUMMY_JOBS_PASSWORD())));
        TestHelper::invokeRequest(&defs, Cmd_ptr(new CtsCmd(CtsCmd::FORCE_DEP_EVAL)));

        node_ptr node = defs.findAbsNode(suite1_family1_a);
        BOOST_CHECK_MESSAGE(node, "Could not find node");
        BOOST_CHECK_MESSAGE(node->state() == NState::COMPLETE,
                            "Complete request should place node in NState::COMPLETE");
        BOOST_CHECK_MESSAGE(suite->state() == NState::ACTIVE,
                            "Expected NState::ACTIVE, but found to be " << NState::toString(suite->state()));

        node_ptr nodeb = defs.findAbsNode(suite1_family1_b);
        BOOST_CHECK_MESSAGE(nodeb, "Could not find node");
        BOOST_CHECK_MESSAGE(nodeb->state() == NState::ACTIVE,
                            "Expected NState::ACTIVE, but found to be " << NState::toString(nodeb->state()));
    ...
    {
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new CompleteCmd("suite1/family1/b", Submittable::DUMMY_JOBS_PASSWORD())));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new CompleteCmd(suite1_family2_aa, Submittable::DUMMY_JOBS_PASSWORD())));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new CompleteCmd(suite1_family2_bb, Submittable::DUMMY_JOBS_PASSWORD())));

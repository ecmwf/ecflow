    ...
    switch (api_) {
        ...
        case CtsCmd::RELOAD_WHITE_LIST_FILE: {
            desc.add_options()(
                ...
                "\n"
                "The white list file (authorisation) is used to verify if a 'user' is allowed to perform a given "
                "command.\n"
                "\n"
                "The file path is specified as the ECF_LISTS variable, and loaded only once by the server (on "
                "*startup*).\n"
                "This means that the file contents can be updated, but the file location cannot change during the "
                "server execution.\n"
                "\n"
                ...
                "considering that if the file is not present or is empty (i.e., just contains the version number)\n"
                "then all users have read/write access.\n"
                "\n"
                "On reload, the server will raise an error if file does not exist, or if the content is invalid.\n"
                "\n"
        ...
        case CtsCmd::RELOAD_PASSWD_FILE: {
            desc.add_options()(
                ...
                "\n"
                "The password file (authentication) is used by the server to authenticate a 'user' by verifying\n"
                "if the password provided by the user matches the one held by the server.\n"
                "The password file is also used on the client to automatically load the password for the 'user'\n"
                "when connecting to the server.\n"
                "\n"
                "When the server is configured to use a password file, then ALL users must have a password.\n"
                "\n"
                "The file path is specified as the ECF_PASSWD environment variable, both for the client and server,\n"
                "and is loaded only by the server on *startup*.\n"
                "This means that the file contents can be updated (i.e., add/remove users), but the file location\n"
                ...
                "\n"
                "The ECF_PASSWD environment variable is used to specify the password file location, considering that\n"
                ...
                "   If an empty password file (i.e., containing just the version) is used, no access is allowed.\n"
                " - On the client, the permissions of password file must be set for reading by the 'user' only\n"
                "\n"
                "Usage:\n"
                " --reloadpasswdfile");
            break;
        ...
        case CtsCmd::RELOAD_CUSTOM_PASSWD_FILE: {
            desc.add_options()(
                ...
                "verifying if the password provided by the user matches the one held by the server. This particular\n"
                "file is used for authenticating users that explicitly specify the user name (either via the\n"
                "environment variable ECF_USER or the --user option).\n"
                "\n"
                "This mechanism should be used when most users use the machine login name, but a few users specify\n"
                "their own user name, in which case the password must also be explicitly provided.\n"
                "\n"
                "The file path is specified as the ECF_CUSTOM_PASSWD environment variable, both for the client and\n"
                "server, and is loaded only by the server on *startup*.\n"
                "This means that the file contents can be updated (i.e., add/remove users), but the file location\n"
                "cannot change during the server execution.\n"
                "\n"
                "The server automatically loads the password file content as part of the startup procedure.\n"
                "\n"
                "The ECF_CUSTOM_PASSWD environment variable is used to specify the password file location, considering "
                "that\n"

...
BOOST_AUTO_TEST_CASE(test_alter_cmd_for_clock_type_hybrid) {
    ...
    { // Change clock type
      // =====================================================================================================
        ...
        BOOST_CHECK_MESSAGE(s->calendar().month() == month,
                            "Calendar should be updated after re-queue/begin. Expected " << month << " but found "
                                                                                         << s->calendar().month());
...
BOOST_AUTO_TEST_CASE(test_alter_cmd_for_clock_sync) {
    ...
    { // Change clock type
      // =====================================================================================================
        ...
        BOOST_CHECK_MESSAGE(s->calendar().day_of_month() == 1,
                            "Calendar should be updated after re-queue/begin. Expected day of month "
                                << 1 << " but found " << s->calendar().day_of_month());
        BOOST_CHECK_MESSAGE(s->calendar().month() == 1,
                            "Calendar should be updated after re-queue/begin. Expected month "
                                << 1 << " but found " << s->calendar().month());
        BOOST_CHECK_MESSAGE(s->calendar().year() == 2009,
                            "Calendar should be updated after re-queued/begin, Expected year " << 2009 << " but found "
                                                                                               << s->calendar().year());
        BOOST_CHECK_MESSAGE(defs.checkInvariants(error_msg), "checkInvariants failed " << error_msg);
    ...
    { // Change clock gain
      // =====================================================================================================
        ...
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(suitename, AlterCmd::CLOCK_GAIN, "86400", ""))); // add 24 hours in seconds
        clock_ptr v = s->clockAttr();
        BOOST_CHECK_MESSAGE(v.get(), "expected clock to be added");
        ...
        // Check that calendar is updated by the alter
        {
            ...
            BOOST_CHECK_MESSAGE(s->calendar().day_of_month() == day_of_month,
                                "Calendar should be updated after alter. Expected " << day_of_month << " but found "
                                                                                    << s->calendar().day_of_month());
            BOOST_CHECK_MESSAGE(s->calendar().month() == month,
                                "Calendar should be updated after alter. Expected " << month << " but found "
                                                                                    << s->calendar().month());
            ...
            // Now re sync with the computers clock,When we reque, the suite calendar should align to todays date and
            // time
            TestHelper::invokeRequest(&defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::CLOCK_SYNC, "", "")));
...
BOOST_AUTO_TEST_CASE(test_alter_cmd) {
    ...
    { // Change clock type
      // =====================================================================================================
        ...
        BOOST_CHECK_MESSAGE(s->calendar().hybrid() == s->clockAttr()->hybrid(),
                            "Calendar and Clock attribute must be in sync after begin");
        ...
        BOOST_CHECK_MESSAGE(defs.get_edit_history(s->absNodePath()).size() == 1,
                            "expected edit_history of 1 to be added but found "
                                << defs.get_edit_history(s->absNodePath()).size());

        // Check that calendar is in sync with clock attribute after the alter
        {
            BOOST_CHECK_MESSAGE(s->calendar().hybrid() == s->clockAttr()->hybrid(),
                                "Calendar and Clock attribute must be in sync after alter");
            BOOST_CHECK_MESSAGE(s->calendar().hybrid() == true,
                                "expected calendar to be re-initialised to be hybrid after alter");
        }

        TestHelper::invokeRequest(&defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::CLOCK_TYPE, "real", "")));
        BOOST_CHECK_MESSAGE(v && !v->hybrid(), "expected clock to be real");
        BOOST_CHECK_MESSAGE(defs.get_edit_history(s->absNodePath()).size() == 2,
                            "expected edit_history of 2 to be added but found "
                                << defs.get_edit_history(s->absNodePath()).size());
        ...
        // Check that calendar is in sync with clock attribute after the alter
        {
            BOOST_CHECK_MESSAGE(s->calendar().hybrid() == s->clockAttr()->hybrid(),
                                "Calendar and Clock attribute must be in sync after alter");
            BOOST_CHECK_MESSAGE(s->calendar().hybrid() == false,
                                "expected calendar to be re-initialised to be real after alter");
        }
    }
    ...
    { // Change clock date
      // =====================================================================================================
        ...
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::CLOCK_DATE, "12.12.2012", "")));
        clock_ptr v = s->clockAttr();
        BOOST_CHECK_MESSAGE((v->day() == 12 && v->month() == 12 && v->year() == 2012),
                            "expected clock date to be 12.12.2012 but found " << v->toString());
        BOOST_CHECK_MESSAGE(defs.get_edit_history(s->absNodePath()).size() == 3,
                            "expected edit_history of 3 to be added but found "
                                << defs.get_edit_history(s->absNodePath()).size());
    ...
    { // Change clock gain
      // =====================================================================================================
        ...
        TestHelper::invokeRequest(
            &defs,
            Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::CLOCK_GAIN, "86400", ""))); // add 24 hours in seconds
        clock_ptr v = s->clockAttr();
        ...
        BOOST_CHECK_MESSAGE(defs.get_edit_history(s->absNodePath()).size() == 4,
                            "expected edit_history of 4 to be added but found "
                                << defs.get_edit_history(s->absNodePath()).size());
    ...
    // test add, the deleting of a specific attribute
    {
        ...
        BOOST_CHECK_MESSAGE(task->timeVec().size() == 1,
                            "expected 1 time to be added but found " << s->timeVec().size());
        BOOST_CHECK_MESSAGE(task->todayVec().size() == 1,
                            "expected 1 today attr, to be added but found " << s->todayVec().size());
        BOOST_CHECK_MESSAGE(defs.get_edit_history(task->absNodePath()).size() == 4,
                            "expected edit_history of 4 to be added but found "
                                << defs.get_edit_history(task->absNodePath()).size());
    }
    {
        TestStateChanged changed(s);
        ...
        BOOST_CHECK_MESSAGE(defs.get_edit_history(task->absNodePath()).size() == 8,
                            "expected edit_history of 8 to be added but found "
                                << defs.get_edit_history(task->absNodePath()).size());
    }
    ...
    { // test delete all
        ...
        BOOST_CHECK_MESSAGE(defs.get_edit_history(task->absNodePath()).size() == 10,
                            "expected edit_history of 10 to be added but found "
                                << defs.get_edit_history(task->absNodePath()).size());
        ...
        BOOST_CHECK_MESSAGE(defs.get_edit_history(task->absNodePath()).size() == 10,
                            "expected edit_history to be truncated to 10, but found "
                                << defs.get_edit_history(task->absNodePath()).size());
    }
    ...
    { // test add variables
        TestStateChanged changed(s);
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_VARIABLE, "FRED1", "_val_")));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_VARIABLE, "FRED2", "_val_")));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_VARIABLE, "FRED3", "_val_")));
    ...
    { // test add variable to multiple nodes
        ...
        for (auto& node : nodes) {
            paths.push_back(node->absNodePath());
        }
        BOOST_CHECK_MESSAGE(paths.size() >= 2, "expected at least 2 nodes");

        for (auto& node : nodes) {
            if (node->variables().size() > 0) {
                TestHelper::invokeRequest(&defs, Cmd_ptr(new AlterCmd(node->absNodePath(), AlterCmd::DEL_VARIABLE)));
            }
        }
        ...
        for (auto& node : nodes) {
            BOOST_CHECK_MESSAGE(node->variables().size() == 3,
                                "expected 3 variable but found " << node->variables().size());
        }
        ...
        for (auto& all_node : nodes) {
            BOOST_CHECK_MESSAGE(all_node->variables().size() == 2,
                                "expected 2 variable but found " << all_node->variables().size());
        }

        TestHelper::invokeRequest(&defs, Cmd_ptr(new AlterCmd(paths, AlterCmd::DEL_VARIABLE)));
        for (auto& all_node : nodes) {
            BOOST_CHECK_MESSAGE(all_node->variables().size() == 0,
                                "expected 0 variable but found " << all_node->variables().size());
        }
    }
    ...
    { // test add label
        TestStateChanged changed(s);
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LABEL, "label", "labelValue")));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LABEL, "label1", "labelValue")));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LABEL, "label2", "labelValue")));
    ...
    { // test add Trigger
        ...
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(task->absNodePath(), AlterCmd::TRIGGER, "x == complete", "")));

        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(task->absNodePath(), AlterCmd::TRIGGER, "t2 == complete", "")));
        BOOST_CHECK_MESSAGE(task->triggerExpression() == "trigger t2 == complete",
                            "expected trigger to be changed found " << task->triggerExpression());
    }
    ...
    { // test add complete expression
        ...
        task->add_complete("t1 == complete");
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(task->absNodePath(), AlterCmd::COMPLETE, "x == complete", "")));

        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(task->absNodePath(), AlterCmd::COMPLETE, "t2 == complete", "")));
        BOOST_CHECK_MESSAGE(task->completeExpression() == "complete t2 == complete",
                            "expected complete expression to be changed found " << task->completeExpression());
    }
    ...
    { // test add limit
        ...
        paths.insert("made_up_path");
        Limit limit_path("limit_name", 10);
        limit_path.set_paths(paths);

        s->addLimit(limit_path);
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_LIMIT_PATH, "limit_name", "made_up_path")));
    ...
    { // test add in-limit
        ...
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "limit_name1", "11")));
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/path/to/limit:limit_name2")));
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/path/to/limit:limit_name3", "10")));
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/path/to/limit:limitA", "10")));
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/path/to/limit/a:limitA", "10")));
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/path/to/limit/aa:limitA", "10")));
        ...
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_INLIMIT, "/path/to/limit:limitA")));
        BOOST_CHECK_MESSAGE(s->inlimits().size() == 5, "expected 5 but found " << s->inlimits().size());
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_INLIMIT, "/path/to/limit/a:limitA")));
        BOOST_CHECK_MESSAGE(s->inlimits().size() == 4, "expected 4 but found " << s->inlimits().size());
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_INLIMIT, "/path/to/limit/aa:limitA")));
    ...
    { // test add repeat
        ...
        BOOST_CHECK_MESSAGE(s->repeat().valueAsString() == "20090917",
                            "expected 20090917 but found " << s->repeat().valueAsString());
    }
    {
        s->deleteRepeat();
        TestStateChanged changed(s);
        ...
        BOOST_CHECK_MESSAGE(s->repeat().valueAsString() == "20090930",
                            "expected 20090930 but found " << s->repeat().valueAsString());
    }
    {
        TestStateChanged changed(s);
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_REPEAT))); // delete previous repeat
        ...
        BOOST_CHECK_MESSAGE(s->repeat().valueAsString() == "b",
                            "expected 'b' but found " << s->repeat().valueAsString());
        TestHelper::invokeRequest(&defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::REPEAT, "c", "")));
        BOOST_CHECK_MESSAGE(s->repeat().valueAsString() == "c",
                            "expected 'c' but found " << s->repeat().valueAsString());
    }
    {
        TestStateChanged changed(s);
        TestHelper::invokeRequest(
...

    {
        ...
        BOOST_CHECK_MESSAGE(s->repeat().valueAsString() == "22",
                            "expected '22' but found " << s->repeat().valueAsString());
    }
    ...
    { // test suite changed
        ...
        // test add zombie
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_ZOMBIE, "user:fob:init:300")));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_ZOMBIE, "path:fob:init:300")));
        TestHelper::invokeRequest(&defs,
                                  Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_ZOMBIE, "ecf:fob:init:300")));
    ...
    BOOST_CHECK_MESSAGE(defs.get_edit_history(s->absNodePath()).size() == 10,
                        "expected edit_history to be truncated to 10, but found "
                            << defs.get_edit_history(s->absNodePath()).size());

    {
        ...
        BOOST_CHECK_MESSAGE(defs.get_edit_history().size() == 0,
                            "expected edit_history to be cleared but found" << defs.get_edit_history().size());
    }
    ...
    { // test add late
        TestStateChanged changed(s);
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LATE, "late -s 10:10 -a 23:10 -c +23:10")));
        ...
        BOOST_CHECK_MESSAGE(s->get_late() && s->get_late()->toString() == "late -s +10:10",
                            "expected 'late -s 10:10' but found " << s->get_late()->toString());
    ...
    }
    {
        TestStateChanged changed(s);
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LATE, "-s 10:10 -a 23:10 -c +23:10")));
        BOOST_CHECK_MESSAGE(s->get_late(), "expected late to be added");

        // test change late
        TestHelper::invokeRequest(&defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::LATE, "-s 10:10 -a 12:00")));
        BOOST_CHECK_MESSAGE(s->get_late() && s->get_late()->toString() == "late -s +10:10 -a 12:00",
                            "expected 'late -s +10:10 -a 12:00' but found " << s->get_late()->toString());
    ...
    // ================================ TIME ==================================================================
    {
        ...
        BOOST_CHECK_MESSAGE(task->timeVec()[0].toString() == "time 10:10",
                            "expected 'time 10:10' but found " << task->timeVec()[0].toString());
    ...
    // ================================ TODAY ==================================================================
    {
        ...
        TestStateChanged changed(s);
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(task->absNodePath(), AlterCmd::ADD_TODAY, "+10:10 23:00 00:01")));
        BOOST_CHECK_MESSAGE(task->todayVec().size() == 1, "expected 1 today but found " << task->todayVec().size());

        // test change today
        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(task->absNodePath(), AlterCmd::TODAY, "+10:10 23:00 00:01", "+10:10")));
        BOOST_CHECK_MESSAGE(task->todayVec()[0].toString() == "today +10:10",
                            "expected 'today +10:10' but found " << task->todayVec()[0].toString());
...
BOOST_AUTO_TEST_CASE(test_alter_sort_attributes) {
    ...
    add_sortable_attributes(f1.get());
    task_ptr t1 = f1->add_task("t1");
    add_sortable_attributes(t1.get());
...
BOOST_AUTO_TEST_CASE(test_alter_sort_attributes_for_task) {
    ...
    suite_ptr ss = sorted_defs.add_suite("suite");
    add_sortable_attributes(ss.get());
    family_ptr sf1 = ss->add_family("f1");
...
BOOST_AUTO_TEST_CASE(test_alter_cmd_errors) {
    ...
    { // test add variables
        ...
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LIMIT, "limit"))); // no limit value
        TestHelper::invokeFailureRequest(
            &defs,
            Cmd_ptr(new AlterCmd(
                s->absNodePath(), AlterCmd::ADD_LIMIT, "limit", "xx"))); // value not convertible to integer
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_LIMIT, ".1", "10"))); // not a valid name

        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, ""))); // no inlimit value
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/limit"))); // limit path, but no name
        TestHelper::invokeFailureRequest(&defs,
                                         Cmd_ptr(new AlterCmd(s->absNodePath(),
                                                              AlterCmd::ADD_INLIMIT,
                                                              "/path/tolimit:limit",
                                                              "xx"))); // tokens must be convertible to an integer

        TestHelper::invokeRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::ADD_INLIMIT, "/path/tolimit:limit", "1")));
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_INLIMIT, "/path/tolimit"))); // no limit name
        TestHelper::invokeFailureRequest(
            &defs, Cmd_ptr(new AlterCmd(s->absNodePath(), AlterCmd::DEL_INLIMIT, "/path/tolimit:"))); // no limit name
        TestHelper::invokeFailureRequest(
            &defs,
            Cmd_ptr(
                new AlterCmd(s->absNodePath(), AlterCmd::DEL_INLIMIT, "/path/tolimit:12 34"))); // invalid limit name

...
#ifndef ecflow_node_ExprParserV2_HPP
#define ecflow_node_ExprParserV2_HPP
...
///
/// @details This is the handcrafted implementation that replaces the Boost.Spirit Classic grammar
/// used by ecf::expression::v1::ExprParser. The implementation is a recursive-descent parser with
/// a hand-written lexer, producing an identical Ast* tree to V1 for every accepted expression.
///
/// The ExprDuplicate cache is consulted first so identical expressions incur the parse cost only
/// once per process lifetime.
///
/// @invariant After a successful doParse(), getAst() returns a non-null pointer and the returned
///            ast() unique_ptr is non-null until moved.
///
class ExprParser {
public:
    ///
...
#endif /* ecflow_node_ExprParserV2_HPP */

...
void Task::requeue(Requeue_args& args, const ecf::AuthorisationContext& authorisation) {
    if (aliases_.empty()) {
        if (alias_no_ != 0) {
            reset_alias_number();
        }
...
void Task::collateChanges(DefsDelta& changes, const ecf::AuthorisationContext& ctx) const {

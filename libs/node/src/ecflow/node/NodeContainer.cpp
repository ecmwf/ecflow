...
void NodeContainer::requeue(Requeue_args& args, const ecf::AuthorisationContext& authorisation) {
...
void NodeContainer::collateChanges(DefsDelta& changes, const ecf::AuthorisationContext& ctx) const {
...
void NodeContainer::setStateOnlyHierarchically(NState::State s, const AuthorisationContext& ctx, bool force) {
...
void NodeContainer::set_state_hierarchically(NState::State s, const AuthorisationContext& ctx, bool force) {
    if (!ctx.allows(this->absNodePath(), ecf::Allowed::WRITE)) {
        return;
    }

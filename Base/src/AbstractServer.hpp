...
public:
   ...
   /// HALTED       yes               no               no             no
	...
	/// This can be called at the end of a child command, hence time_now may be >= next_poll_time
	/// If this is the case, we will defer job generation
	virtual void traverse_node_tree_and_job_generate(const boost::posix_time::ptime& time_now,bool cmd_context) const = 0;

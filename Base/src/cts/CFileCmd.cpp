...
STC_Cmd_ptr CFileCmd::doHandleRequest(AbstractServer* as) const
{
 		...
 		switch (file_) {
			...
			case CFileCmd::JOBOUT: {
				...
				if (!File::open(ecf_jobout_gen_var.theValue(),fileContents)) {
					std::stringstream ss;
					ss << "CFileCmd::doHandleRequest: Failed to open the job-out file('" << ecf_jobout_gen_var.theValue() << "') for task " << pathToNode_;
					throw std::runtime_error( ss.str() ) ;
 				}
				break; }
	...
	if (Str::truncate_at_start(fileContents, max_lines_)) {
		std::stringstream ss;
		ss << "# >>>>>>>> File truncated down to " << max_lines_ << ". Truncated from the start <<<<<<<<<\n";
		fileContents += ss.str();
	}

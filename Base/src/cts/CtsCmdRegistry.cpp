...
CtsCmdRegistry::CtsCmdRegistry(bool addGroupCmd)
{
   ...
   vec_.push_back( std::make_shared<QueryCmd>());
	...
	if (addGroupCmd) vec_.push_back( std::make_shared<GroupCTSCmd>());
}


static void PrintVariableMap(const boost::program_options::variables_map& vm) {
	std::cout << "boost::program_options::variables_map:    vm.size() " << vm.size() << "\n";
                           ...
                           AbstractClientEnv* clientEnv ) const
{
	for(const auto& registered_cmd: vec_) {
		...
		if ( vm.count( registered_cmd->theArg() ) ) {
			...
			if (clientEnv->debug()) std::cout << "  CtsCmdRegistry::parse matched with registered command " << registered_cmd->theArg() << "\n";

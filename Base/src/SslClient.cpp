  ...
  timeout_(timeout)
{
   ...
   if (!cmd_ptr.get())  throw std::runtime_error("SslClient::SslClient: No request specified !");
...
#ifdef DEBUG_CLIENT
   std::cout << "   SslClient::SslClient() timeout(" << timeout_ << ") " << host_ << ":" << port_ << " "; cmd_ptr->print(std::cout); std::cout << std::endl;
#endif
...
SslClient::~SslClient() {
#ifdef DEBUG_CLIENT
   std::cout << "   SslClient::~SslClient(): connection_.socket().is_open()=" << connection_.socket_ll().is_open() << std::endl;
#endif
}
                        ...
                        boost::asio::ip::tcp::resolver::iterator endpoint_iterator )
{
  ...
  if (!connection_.socket_ll().is_open())
  {
     ...
     if (!start_connect( ++endpoint_iterator)) {
        ...
        stop();
        std::stringstream ss;
        if (e) ss << "SslClient::handle_connect: Ran out of end points : connection error( " << e.message() << " ) for request( " << outbound_request_ << " ) on " << host_ << ":" << port_;
        else   ss << "SslClient::handle_connect: Ran out of end points : connection error for request( " << outbound_request_ << " ) on " << host_ << ":" << port_;
...
#ifdef DEBUG_CLIENT
     std::cout << "   SslClient::handle_connect Connect error: " << e.message() << " . Trying next end point" << std::endl;
#endif
     ...
     if (!start_connect( ++endpoint_iterator)) {
        // Ran out of end points. An error occurred.
        stop();
        std::stringstream ss; ss << "SslClient::handle_connect: Ran out of end points: connection error( " << e.message() << " ) for request( " << outbound_request_ << " ) on " << host_ << ":" << port_;
        throw std::runtime_error(ss.str());
     }
  }
  else {
#ifdef DEBUG_CLIENT
     std::cout << "   SslClient::handle_connect **Successfully** established connection to the server: Sending Out bound request = " << outbound_request_ << std::endl;
#endif
...
void SslClient::handle_handshake( const boost::system::error_code& e )
{
   ...
   if (!e) {
      ...
      std::stringstream ss; ss << "SslClient::handle_handshake: error(" << e.message() << ") on " << host_ << ":" <<  port_ << " possibly non-ssl server";
...
void SslClient::handle_write( const boost::system::error_code& e )
{
   ...
   if ( !e ) {
      ...
      std::stringstream ss;  ss << "SslClient::handle_write: error (" << e.message() << " ) for request( " << outbound_request_ << " ) on " << host_ << ":" <<  port_;
      throw std::runtime_error(ss.str());
   }
...
{
   if (debug) std::cout << "  SslClient::handle_server_response" << std::endl;
...
{
#ifdef DEBUG_CLIENT
      std::cout << "   SslClient::check_deadline stopped_=" << stopped_
   ...
   if (deadline_.expires_at() <= boost::asio::deadline_timer::traits_type::now())
   {
      ...
      std::stringstream ss;
      ss << "SslClient::check_deadline: timed out after " << timeout_ << " seconds for request( " << outbound_request_ << " ) on " << host_ << ":" << port_;
      throw std::runtime_error(ss.str());
   }

...
BOOST_AUTO_TEST_CASE(test_query_cmd) {
    ...
    TestHelper::invokeFailureRequest(
        &defs, Cmd_ptr(new QueryCmd("repeat", "/suite/f/t1", "", "/suite/f/t1"))); // no repeat on /suite/f/t1
    TestHelper::invokeFailureRequest(
        &defs, Cmd_ptr(new QueryCmd("repeat", "/suite", "fred", "/suite"))); // Only next | prev are valid
    ...
    TestHelper::invokeFailureRequest(
        &defs, Cmd_ptr(new QueryCmd("label", "/suite/f/t1", label_name, "/suite/f/t1"))); // no label on t1
    TestHelper::invokeFailureRequest(
        &defs, Cmd_ptr(new QueryCmd("label", "/suite/f/t2", "fred", "/suite/f/t2"))); // wrong label name
    ...
    TestHelper::invokeFailureRequest(&defs,
                                     Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "t3 == complete", "/suite/f/t1")));
    ...
    // Note: we pick a task outside of a repeat, since setting a task to complete, inside a repeat will cause it to
    // requeue Avoid using ForceCmd to avoid side affects
    std::vector<std::string> states = NState::allStates();
    for (const auto& state : states) {
        task->setStateOnly(NState::toState(state));
        res = TestHelper::invokeRequest(
            &defs, Cmd_ptr(new QueryCmd("state", task->absNodePath(), "", task->absNodePath())), false);
    ...
    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("dstate", task->absNodePath(), "", task->absNodePath())), false);
    BOOST_CHECK_MESSAGE(res == "suspended", "expected query state to return suspend but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("event", "/suite/f/t1", event_name, "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "clear", "expected query event to return clear but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("meter", "/suite/f/t1", meter_name, "/suite/f/t1")), false);
    ...
    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("limit_max", "/suite", limit_name, "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "12", "expected query limit_max to return 12 but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("label", "/suite/f/t2", label_name, "/suite/f/t2")), false);
    BOOST_CHECK_MESSAGE(res == "value", "expected query label to return 'value' but found: " << res);
    res = TestHelper::invokeRequest(&defs,
    ...
    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "t2 == complete", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "false", "expected query trigger to return false but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "1 == 1", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "true", "expected query trigger to return true but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "/suite/f/t1:var1 == 0", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "true", "expected query trigger to return true but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "/suite/f:var2 == 0", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "true", "expected query trigger to return true but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "/suite/f/t1:m == 0", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "true", "expected query trigger to return true but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite/f/t1", "/suite/f/t1:event", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "false", "expected query trigger to return false but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("trigger", "/suite", "/suite:YMD == 20090916", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "true", "expected query trigger to return true but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("variable", "/suite/f/t1", "var1", "/suite/f/t1")), false);
    BOOST_CHECK_MESSAGE(res == "var1", "expected query variable to return var2 but found: " << res);

    res = TestHelper::invokeRequest(
        &defs, Cmd_ptr(new QueryCmd("variable", "/suite/f/t1", "var2", "/suite/f/t1")), false);

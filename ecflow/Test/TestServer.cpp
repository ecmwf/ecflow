...
BOOST_AUTO_TEST_CASE( test_server_job_submission )
{
   ...
   TestClean clean_at_start_and_end;
	...
	int taskSize = 3; // on linux 1024 tasks take ~4 seconds for job submission
  	Defs theDefs;
 	{
		std::auto_ptr< Suite > suite( new Suite( "test_server_job_submission" ) );
...
BOOST_AUTO_TEST_CASE( test_restore_defs_from_check_pt )
{
   ...
   BOOST_REQUIRE_MESSAGE( theClient.getDefs() == 0, "Expected getDefs() to succeed\n");
   BOOST_REQUIRE_MESSAGE( theClient.defs()->suiteVec().size() == 1, "Expected 1 suite at the start\n");

   // Check pt, make sure file exist on disk and has a non zero file size\n";
   BOOST_REQUIRE_MESSAGE( theClient.checkPtDefs() == 0,"Expected Check-pt to succeed.\n" << theClient.errorMsg());
   ...
   BOOST_REQUIRE_MESSAGE( theClient.restoreDefsFromCheckPt() == 1,"Expected restoreDefsFromCheckPt to *FAIL* since server not halted\n");
   BOOST_REQUIRE_MESSAGE( theClient.haltServer() == 0,"Expected halt server to succeed\n" << theClient.errorMsg());
   BOOST_REQUIRE_MESSAGE( theClient.restoreDefsFromCheckPt() == 1,"Expected restoreDefsFromCheckPt to *FAIL* since definition not deleted yet\n");
   ...
   BOOST_REQUIRE_MESSAGE( theClient.defs()->suiteVec().empty(), "Expected no suites, after delete_all()\n");
   ...
   BOOST_REQUIRE_MESSAGE( theClient.restoreDefsFromCheckPt() == 0,"Expected restoreDefsFromCheckPt to *SUCCEED*, since server halted and defs deleted \n" << theClient.errorMsg());
   BOOST_REQUIRE_MESSAGE( theClient.getDefs() == 0, "Get defs failed should return 0\n" << theClient.errorMsg());
   BOOST_REQUIRE_MESSAGE( theClient.defs()->suiteVec().size() == 1, "Restore from check pt did not work. expected 1 suite but found " << theClient.defs()->suiteVec().size() );

   // restore default check pointing, for test that follow
   BOOST_REQUIRE_MESSAGE(theClient.checkPtDefs(ecf::CheckPt::ON_TIME,120) == 0,CtsApi::checkPtDefs(ecf::CheckPt::ON_TIME) << " failed should return 0\n" << theClient.errorMsg());

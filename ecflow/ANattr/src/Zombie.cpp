...
{
   if (user_action_set_) return user_action_;
   return attr_.action();
}
...
std::ostream& operator<<(std::ostream& os, const Zombie& z)
{
	...
	os << z.process_or_remote_id() << "<pid> ";
 	...
 	os << "calls(" << z.calls() << ") ";
 	if (z.fob()) os << "FOB";
 	else if (z.fail()) os << "FAIL";
 	else if (z.adopt()) os << "ADOPT";
 	else if (z.block()) os << "BLOCK";
   else if (z.remove()) os << "REMOVE";
   else if (z.kill()) os << "KILL";
 	switch (z.last_child_cmd()) {
 		case Child::INIT: os << " INIT"; break;
 		case Child::EVENT: os << " EVENT"; break;
 		case Child::METER: os << " METER"; break;
 		case Child::LABEL: os << " LABEL"; break;
 		case Child::WAIT: os << " WAIT"; break;
 		case Child::ABORT: os << " ABORT"; break;
 		case Child::COMPLETE: os << " COMPLETE"; break;
 	}
	return os;
}
			  ...
			  int indent)
{
   ...
   size_t user_action_width = 6;   // max of FOB,FAIL,ADOPT,BLOCK,REMOVE
   ...
   BOOST_FOREACH(const Zombie& z, zombies) {
     ...
     if (indent != 0) for(int i = 0; i < indent; i++) ss << " ";
      ss << left << setw(path_width)         << z.path_to_task() << " "
                ...
                << setw(user_action_width)   << User::to_string(z.user_action()) << " "

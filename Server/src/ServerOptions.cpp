...
( "help,h",
         ...
         "  Note: Any settings will be prepended with <host>.<port>.\n"
         "ECF_PROFILE_THRESHOLDS:\n"
         "  The Job generation process is expected to take less than 60 seconds\n"
         "  Times greater than 60 seconds can interfere with honouring time dependencies\n"
         "  and also interfere with client/server communication, i.e. causing timeouts in the client\n"
         "  The causes can be:\n"
         "    - Slow disk I/0, or when server is run an a virtual machine\n"
         "    - Insufficient memory, when compared to the size of the scripts/definition\n"
         "    - Large number of jobs submitted at the same time, and/or very large scripts\n"
         "  When the process takes longer than 60 seconds, an error message is issued, to the log file\n"
         "  Lookout for text 'Jobs::generate: job generation time' in the log file.\n"
         "  Additional information is also output helping to determine the cause.\n"
         "  Timers record the cost of Job generation for each suite/family/task\n"
         "  To stop being overwhelmed we employ thresholds, for each kind of node\n"
         "  Hence *only* job generation that takes longer than the threshold is output to the log file\n"
         "  Default is 'suite:2000,family:1000,task:300', where the integer represents milliseconds\n"
         "  This environment allows the thresholds to be configured i.e\n"
         "  export ECF_PROFILE_THRESHOLDS=suite:2500,family:1000,task:200\n"
         "\n"

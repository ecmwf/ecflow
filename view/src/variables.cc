...
void search_item (Widget text_w, XtPointer client_data,
		  XtPointer call_data, Widget nam, Widget val)
{
  ...
  while (cnt--) {
    if (!(text = (char *) xec_GetString(strlist[cnt])))
      break;
    ...
    if (re_exec (text) > 0) {
		...
		while(*r && *r != '=')		  r++;
		*r = 0;
		while(*q && q[strlen(q) - 1] == ' ') q[strlen(q) - 1] = 0;
		r += 2;		
		if(*p == '(')
		  r[strlen(r)-1] = 0;
		while(*r && r[strlen(r) - 1] == ' ')
...
void variables::setCB( Widget, XtPointer )
{
  if(get_node()) {
	...
	{
	  ok = confirm::ask(True,
			    "This variable is already defined in the %s %s\n"
	...
	if (get_node()->__node__())
	  get_node()->serv().command
	    (clientName, "--alter", add ? "add" : "change", "variable", 
	     name, value, get_node()->full_name().c_str(), NULL);
	else
	  get_node()->serv().command
	    ("alter", "-v", get_node()->full_name().c_str(), name, value, NULL);
	if (add) update();
      }
      XtFree(name);
      XtFree(value);      
...
void variables::nameCB( Widget, XtPointer )
{
	...
	if(get_node())
	{
		...
		XtSetSensitive(delete_, 
			n != 0 && (!n->isGenVariable(p) || n != get_node()));
	}
	else {
		clear();
	}

...
class ecf_node_builder : public ecf_node_maker {
  virtual node* make(host& h, ecf_node* e) { return new W(h,e); }
public:
...
class ExpressionWrapper {
 ...
 public:
  ...
  std::string expression() const { 
    if (kind_ == 'c')
      return node_->completeExpression();
    return node_->triggerExpression();   
  }
  
  AstTop* get_ast_top() {
    if (kind_ == 'c')
      return node_->completeAst();
...
class ecf_node : public observable {
 ...
 public:
   ...
   virtual bool is_late() { return false; }
   virtual bool hasZombieAttr() { return false; }
   virtual bool hasTime() { return false; }
   virtual bool hasDate() { return false; }
   virtual bool hasTrigger() { return false; }

   virtual std::ostream& print(std::ostream& s) const { return s << none(); }
   virtual void print(std::ostream& s) { s << none(); }
   virtual const std::string& variable(const std::string& name) const = 0;
   virtual void why(std::ostream &) const {};
   virtual std::string get_var(const std::string& name, 
                               bool is_gen=false, 
 ...
 public:
   ecf_node*  parent() const { return parent_; }
   node* xnode() const { return node_; }
   char kind() { return kind_; }
...
class ecf_concrete_node : public ecf_node, public AbstractObserver 
{
 ...
 ecf_concrete_node(T* owner,ecf_node* parent, char c = 'd')
    ...
    , owner_(owner)
  {
   ...
   ~ecf_concrete_node() { unlink(); }

   virtual void set_graphic_ptr(node*) {};

   T* get() const { return owner_;} 
   ...
   virtual const std::string& name() const { return ecf_node::name(); }
   virtual const std::string& full_name() const { return full_name_; }
   virtual bool is_late() { return false; }
   virtual bool hasZombieAttr() { return false; }
   virtual bool hasTime() { return false; }
   ...
   virtual const std::string type_name() const { return typeid(owner_).name(); }

   virtual void update(const Node* n, const std::vector<ecf::Aspect::Type>& asp)
   { if (!owner_) return; ecf_node::update(n, asp); }
   virtual void update(const Defs* n, const std::vector<ecf::Aspect::Type>& asp)
   { if (!owner_) return; ecf_node::update(n, asp); }

   void update_delete(const Node* n) { unlink(); ecf_node::update_delete(n); }
   void update_delete(const Defs* n) { unlink(); ecf_node::update_delete(n); }

   virtual const std::string& variable(const std::string&) const { return none(); }
   virtual void why(std::ostream &) const {};
   virtual std::string get_var(const std::string& name, 
                               bool is_gen=false, 
                               bool substitute=false) { 
   ...
   virtual void unlink(bool detach=true) { owner_ = 0x0; ecf_node::unlink(detach); }
   virtual void check() const {
     if (get() == 0x0) std::cerr << "# ecf: no owner: " << name() << "\n";
     ecf_node::check();
   }

   virtual Limit* get_limit(const std::string& name) { return 0x0; }
   virtual Node* get_node() const { return 0x0; }
   virtual const Label& get_label(const std::string& name) { return Label::EMPTY(); }
   virtual const Event& get_event(const std::string& name) { return Event::EMPTY(); }
   virtual const Meter& get_meter(const std::string& name) { return Meter::EMPTY(); }
   virtual const Repeat& get_repeat() { return crd(); }

 private:
   bool is_reset(const std::vector<ecf::Aspect::Type>& v) const
   {
     bool reset = false;
     for (std::vector<ecf::Aspect::Type>::const_iterator it = v.begin(); it != v.end(); ++it) 
       { if (*it == ecf::Aspect::ORDER || *it == ecf::Aspect::ADD_REMOVE_NODE || *it == ecf::Aspect::ADD_REMOVE_ATTR)
           { reset = true; break;} }
     return reset;
   }
...
{
  for(typename std::vector<T>::
        const_reverse_iterator j = v.rbegin(); j != v.rend(); ++j) {
...
{ if (owner_) return owner_->toString(); return ""; }

...
::
   ...
   The following environment variables are used specifically by task commands:
     ...
     ECF_TIMEOUT <int> [optional]
       Maximum time for a task command to deliver a message to the server, retrying the main and alternate hosts until the limit is reached
       Default: 86400 seconds; accepted range: 60 to 86400 seconds
     ECF_ZOMBIE_TIMEOUT <int> [optional]
       Maximum time for a task command flagged as a zombie by the server to keep retrying; the ECF_TIMEOUT limit still applies, so the effective limit is the smaller of the two
       Default: 43200 seconds; accepted range: 60 to 86400 seconds
     ECF_CONNECT_TIMEOUT <int> [optional]
       Maximum time to establish a connection with the server; 0 means that the timeout is taken from the command being executed
       Default: 0 seconds
...
.. seealso::
   ...
   :ref:`Protocol reference for CtsWaitCmd <proto_CtsWaitCmd>` — the request/reply

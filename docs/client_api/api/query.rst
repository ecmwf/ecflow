...
-  arg2 = <path> \| <path>:name where name is name of a event,
   meter,limit or variable. Path ``/`` represents the server itself,
   and can only be used with ``state`` or ``variable``
...
.. seealso::
   ...
   :ref:`Protocol reference for QueryCmd <proto_QueryCmd>` — the request/reply

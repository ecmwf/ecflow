...
.. glossary::
   ...
   job creation
      ...
      - Generating a unique password ECF_PASS, which is placed in :term:`ecf script` during :term:`pre-processing`. See :ref:`head_h`
   ...
   zombie type
      ...
      * **user**: Job is created by :term:`user commands` like, rerun, re-queue. User zombies are differentiated from server(scheduled) since they are automatically created when the force option is used and we have tasks in an :term:`active` or :term:`submitted` states.

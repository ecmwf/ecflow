...
def timing_decorator(function_to_time):
    def scafold(*function_args):
        ...
        print(
            "%s function took %0.3f ms for %s"
            % (function_to_time.func_name, (end - start) * 1000.0, function_args[1])
        )
...
if __name__ == "__main__":
    ...
    print("####################################################################")
    print(
        "Test performance of sync local using "
        + Client().version()
        + " debug build("
        + str(debug_build())
        + ")"
    ...
    for suite in suites:
        CL.ch_register(
            False, [suite.name()]
        )  # register interest in single suite, corresponding handle is attached to CL

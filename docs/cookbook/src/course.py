...
class NativePerl(GenericFamily):
    ...
    def main(self, node):
        tsk = Task(self.name).add(
            Variables(ECF_MICRO="^", ECF_JOB_CMD="^ECF_JOB^ > ^ECF_JOBOUT^ 2>&1"),
            Meter("step", -1, 100),
            Event("1"),
            Event("2"),
            Label("info", "none"),
...
def _kind(prod=1, cons=1):
    return Variables(CONSUME=cons, PRODUCE=prod)
...
def _leaf(name="produce", init=0, stop=100, step=1):
    ...
    return Task("%s" % name).add(
        _evt(),
        add,
        Variables(INIT=init, STOP=stop, STEP=step),
    )
...
class FallBack(GenericFamily):
    ...
    def main(self, node=None):
        return (
            Family("daily")
            .repeat(name="YMD", start=today, end=DATE_STOP)
            .add(
                Task("action").add(Time("10:00")),
                Family("loop").add(
                    Time("11:00"),
                    Task("dummy").add(TriggerImpossible(), Complete("1==1")),
                ),
                Task("fallback").add(
...
class Consume(GenericFamily):
    ...
    def main(self, node):
        ...
        top = node.family("consume").add(
            Variables(SLEEP=10, PRODUCE=1, CONSUME=1),  # default: tasks will do both
            Family("limits").add(Defcomplete(), Limit("consume", 7)),
            Task("leader").add(
        ...
        for step in xrange(self.init, self.stop, self.step):
            fam.add(
                Family("%02d" % step).add(
                    Variables(STEP=step), _leaf("produce", step, step, 1)
                )
            )
        ...
        top.add(  ### trigger may be inside a task
            ...
            .add(
                ...
                _kind(0, 1),
                Trigger(
                    "(consume1:STEP lt %s1:STEP and %s) or " % (prod, lead)
                    + "(consume1:STEP lt %s0/produce:step and not %s) or "
                    % (prod, lead)
                    + "(%s1 eq complete and %s0 eq complete)"  # lt while both are repeat
                    % (prod, prod)
                ),
                InLimit("limits:consume"),
        ...
        for step in xrange(self.init, self.stop, self.step):
            fam.add(
                Family("%02d" % step).add(
                    Variables(STEP=step),
                    Trigger(
                        "(%02d:STEP le %s1:STEP and %s) or " % (step, prod, lead)
...
class Barber(GenericFamily):
    ...
    def _passer_by(self):
        ...
        return Task("passby").add(
            ...
            Label(
                "rem",
                "this task alters its variable ID, " + "aliases won't work natively",
            ),
    ...
    def _client(self, node, position):
        ...
        fam = (
            ...
            .add(
                ...
                Task("leave").add(Label("info", ""), Trigger(["cut", "pay"])),
            )
        )
...
class Course(ie.Seed):
    ...
    def suite(self):
        ...
        node = Suite(user())
        node.defstatus("suspended").add(Variables(USER=user()), locate_scripts())  #
        self.top(node)
    ...
    def top(self, node):
        ...
        with node.family(self.name) as node:
            node.add(FallBack().main(), DailyInc().main())
...
class Admin(Course):
    ...
    def main(self):
        ...
        return (
            Task("newlog").add(Label("info", "renew server log-file"), Time("08:00")),
            Task("logsvr").add(
                Defcomplete(),
                Variables(
                    HOST="pikachu",
            ...
            Family("loop").add(
                Time("08:30"),
                Family("dummy").add(Complete("1==1")),  # TriggerImpossible(),
            ),
        )
...
class EcEvents(Admin):
    ...
    def main(self, node):
        for mode in ["list", "register", "delete", "register_all", "delete_all"]:
            added = None
            if "list" in mode:
                added = Label("regs", "")

            fam = Family(mode).add(
                Variables(MODE=mode), Task("ecjobs").add(Label("info", "none"), added)
            )
            node.add(fam)
            if "_all" in mode:
                ...
                fam.add(
                    Variables(EVENT="an12h000"), Label("info", "update EVENT variable")
                )

        event = "an00h000"
        node.family("ref").defstatus("complete").add(
            Task(event).add(Variables(YMD=today), Label("YMD", today))
        )

        node.family("user").family(event).repeat(
            name="YMD", start=today, end=DATE_STOP
        ).add(
            Label(
                "info",
                "extern cannot be used anymore for " + "intra-suite reference triggers",
            ),
            Variables(SLEEP=1),
...
class SerialTask(object):
    ...
    def add(self, name):
        fam = Family(name).add(Variables(MODE=name), Task("to_ecflow"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", default="3141", help="server port number")
    ...
    if 1:  # enable/disable remote version of 'consume'
        rem = suite.family("remote").add(
            Task("mk_remote").add(
                ...
                Label(
                    "info",
                    "do not forget to create directory structure"

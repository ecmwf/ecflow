...
This technical note is a reference for the commands understood by the ecFlow server. It replaces an
older tabular summary with a per-command reference generated directly from the source, so that the
description of each command, its options, and its wire representation stay consistent with the
implementation.

...
#ifdef UI_VMODELDATA_DEBUG
    UI_FUNCTION_LOG_S(server_)
    UiLogS(server_).dbg() << " number of state changes=" << changeInfo_->stateChangeSuites().size();
#endif
...
void VTreeServer::notifyBeginNodeChange(const VNode* vnode,
    ...
    //---------------------------------------------------------------------
        ...
        for (auto it : aspect) {
            ...
            else if (it == ecf::Aspect::FLAG || it == ecf::Aspect::SUBMITTABLE || it == ecf::Aspect::TODAY ||
                     it == ecf::Aspect::TIME || it == ecf::Aspect::DAY || it == ecf::Aspect::CRON ||
                     it == ecf::Aspect::DATE) {
                if (node && node->isAttrInitialised()) {
                    Q_EMIT nodeChanged(this, node);
                }
            ...
            if (it == ecf::Aspect::METER || it == ecf::Aspect::EVENT || it == ecf::Aspect::LABEL ||
                it == ecf::Aspect::LIMIT || it == ecf::Aspect::EXPR_TRIGGER || it == ecf::Aspect::EXPR_COMPLETE ||
    ...
    if (sh == server_) {
        ...
        if (VAttribute* ita = itemNext->isAttribute()) {
            if (aPrev) {
                ...
                if (ita->parent() == aPrev->parent() && attrFilter_->isSet(aPrev->type()) &&
                    attrFilter_->isSet(ita->type())) {
                    return;
                }
...
VModelData::VModelData(NodeFilterDef* filterDef, AbstractNodeModel* model)
    ...
    connect(this, SIGNAL(filterDeleteEnd()), model_, SLOT(slotFilterDeleteEnd()));

    connect(this, SIGNAL(serverAddBegin(int)), model_, SLOT(slotServerAddBegin(int)));

    connect(this, SIGNAL(serverAddEnd()), model_, SLOT(slotServerAddEnd()));

    connect(
        this, SIGNAL(serverRemoveBegin(VModelServer*, int)), model_, SLOT(slotServerRemoveBegin(VModelServer*, int)));

    connect(this, SIGNAL(serverRemoveEnd(int)), model_, SLOT(slotServerRemoveEnd(int)));
...
void VModelData::connectToModel(VModelServer* d) {
    connect(d, SIGNAL(dataChanged(VModelServer*)), model_, SLOT(slotDataChanged(VModelServer*)));

    connect(d, SIGNAL(beginServerScan(VModelServer*, int)), model_, SLOT(slotBeginServerScan(VModelServer*, int)));

    connect(d, SIGNAL(endServerScan(VModelServer*, int)), model_, SLOT(slotEndServerScan(VModelServer*, int)));

    connect(d, SIGNAL(beginServerClear(VModelServer*, int)), model_, SLOT(slotBeginServerClear(VModelServer*, int)));

    connect(d, SIGNAL(endServerClear(VModelServer*, int)), model_, SLOT(slotEndServerClear(VModelServer*, int)));

    connect(d,
            SIGNAL(serverRenamed(VModelServer*, const std::string&)),
            this,
            SLOT(slotServerRenamed(VModelServer*, const std::string&)));

    // The model relays this signal
    connect(d, SIGNAL(rerender()), model_, SIGNAL(rerender()));
}
...
void VTreeModelData::connectToModel(VModelServer* s) {
    ...
    connect(ts,
            SIGNAL(beginAddRemoveAttributes(VTreeServer*, const VTreeNode*, int, int)),
            model_,
            SLOT(slotBeginAddRemoveAttributes(VTreeServer*, const VTreeNode*, int, int)));

    connect(ts,
            SIGNAL(endAddRemoveAttributes(VTreeServer*, const VTreeNode*, int, int)),
            model_,
            SLOT(slotEndAddRemoveAttributes(VTreeServer*, const VTreeNode*, int, int)));

    connect(ts,
            SIGNAL(nodeChanged(VTreeServer*, const VTreeNode*)),
            model_,
            SLOT(slotNodeChanged(VTreeServer*, const VTreeNode*)));

    connect(ts,
            SIGNAL(attributesChanged(VTreeServer*, const VTreeNode*)),
            model_,
            SLOT(slotAttributesChanged(VTreeServer*, const VTreeNode*)));

    connect(ts,
            SIGNAL(beginFilterUpdateRemove(VTreeServer*, const VTreeNode*, int)),
            model_,
            SLOT(slotBeginFilterUpdateRemove(VTreeServer*, const VTreeNode*, int)));

    connect(ts,
            SIGNAL(endFilterUpdateRemove(VTreeServer*, const VTreeNode*, int)),
            model_,
            SLOT(slotEndFilterUpdateRemove(VTreeServer*, const VTreeNode*, int)));

    connect(ts,
            SIGNAL(beginFilterUpdateAdd(VTreeServer*, const VTreeNode*, int)),
            model_,
            SLOT(slotBeginFilterUpdateAdd(VTreeServer*, const VTreeNode*, int)));

    connect(ts,
            SIGNAL(endFilterUpdateAdd(VTreeServer*, const VTreeNode*, int)),
            model_,
            SLOT(slotEndFilterUpdateAdd(VTreeServer*, const VTreeNode*, int)));

    connect(ts,
            SIGNAL(beginFilterUpdateRemoveTop(VTreeServer*, int)),
            model_,
            SLOT(slotBeginFilterUpdateRemoveTop(VTreeServer*, int)));

    connect(ts,
            SIGNAL(endFilterUpdateRemoveTop(VTreeServer*, int)),
            model_,
            SLOT(slotEndFilterUpdateRemoveTop(VTreeServer*, int)));

    connect(ts,
            SIGNAL(beginFilterUpdateInsertTop(VTreeServer*, int)),
            model_,
            SLOT(slotBeginFilterUpdateInsertTop(VTreeServer*, int)));

    connect(ts,
            SIGNAL(endFilterUpdateInsertTop(VTreeServer*, int)),
            model_,
            SLOT(slotEndFilterUpdateInsertTop(VTreeServer*, int)));
}
...
void VTableModelData::connectToModel(VModelServer* s) {
    ...
    connect(ts,
            SIGNAL(nodeChanged(VTableServer*, const VNode*)),
            model_,
            SLOT(slotNodeChanged(VTableServer*, const VNode*)));
}
...
void VTableModelData::add(ServerHandler* server) {
    ...
    connect(d, SIGNAL(updateBegin()), this, SIGNAL(updateBegin()));

    connect(d, SIGNAL(updateEnd()), this, SIGNAL(updateEnd()));

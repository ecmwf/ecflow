...
ComboMulti::ComboMulti(QWidget* widget) : QComboBox(widget), dpyText_("") {
    ...
    connect(del, SIGNAL(itemChecked()), this, SLOT(slotChecked()));
...
void ComboMultiDelegate::setEditorData(QWidget* editor, const QModelIndex& index) const {
    // set editor data
    auto* myEditor = static_cast<QCheckBox*>(editor);
    myEditor->setText(index.data(Qt::DisplayRole).toString());
    myEditor->setChecked(index.data(Qt::CheckStateRole).toBool());

    connect(myEditor, SIGNAL(stateChanged(int)), this, SLOT(slotEdited(int)));
}

void ComboMultiDelegate::setModelData(QWidget* editor, QAbstractItemModel* model, const QModelIndex& index) const {

...
TreeNodeWidget::TreeNodeWidget(ServerFilter* serverFilter, QWidget* parent)
    : NodeWidget("tree", serverFilter, parent), viewLayoutMode_(StandardLayoutMode), layoutProp_(nullptr) {
    ...
    connect(model_, SIGNAL(firstScanEnded(const VTreeServer*)), this, SLOT(firstScanEnded(const VTreeServer*)));

    connect(bcWidget_, SIGNAL(selected(VInfo_ptr)), this, SLOT(slotSelectionChangedInBc(VInfo_ptr)));

    connect(atts_, SIGNAL(changed()), this, SLOT(slotAttsChanged()));
...
void TreeNodeWidget::setViewLayoutMode(TreeNodeWidget::ViewLayoutMode mode) {
    ...
    // Signals-slots
    connect(
        view_->realObject(), SIGNAL(selectionChanged(VInfo_ptr)), this, SLOT(slotSelectionChangedInView(VInfo_ptr)));

    connect(view_->realObject(),
    ...
    connect(model_, SIGNAL(clearBegun(const VTreeNode*)), view_->realObject(), SLOT(slotSaveExpand(const VTreeNode*)));

    connect(
        model_, SIGNAL(scanEnded(const VTreeNode*)), view_->realObject(), SLOT(slotRestoreExpand(const VTreeNode*)));

    connect(model_, SIGNAL(rerender()), view_->realObject(), SLOT(slotRerender()));

    connect(model_,
            SIGNAL(filterUpdateRemoveBegun(const VTreeNode*)),
            view_->realObject(),
            SLOT(slotSaveExpand(const VTreeNode*)));

    connect(model_,
            SIGNAL(filterUpdateAddEnded(const VTreeNode*)),
            view_->realObject(),
            SLOT(slotRestoreExpand(const VTreeNode*)));
}
...
void TreeNodeWidget::populateDockTitleBar(DashboardDockTitleWidget* tw) {
    ...
    iconFilterMenu_ =
        new VParamFilterMenu(menuIcon, icons_, "Show icons", VParamFilterMenu::ShowMode, VParamFilterMenu::PixmapDecor);
    stateFilterMenu_ = new VParamFilterMenu(
        menuState, states_, "Show statuses", VParamFilterMenu::ShowMode, VParamFilterMenu::ColourDecor);

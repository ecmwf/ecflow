...
public:
    ...
    void runFull(ServerHandler* server,
                 ...
                 bool useCache);
    void runMode(VFile::FetchMode fetchMode,
...
void OutputFileFetchQueueManager::runFull(ServerHandler* server,
                                          VNode* node,
                                          const std::string& fileName,
                                          bool isJobout,
                                          size_t deltaPos,
...
void OutputFileProvider::fetchFileInternal(ServerHandler* server,
                                           VNode* n,
                                           const std::string& fileName,
                                           bool isJobout,
                                           size_t deltaPos,
    ...
    if (isJobout) {
        ...
        reply_->addLogRemarkEntry(
            "This file is <b>not</b> the <b>current</b> job output (defined by <b>ECF_JOBOUT</b>).");
    }
...
void OutputFileProvider::fetchFileForMode(VFile_ptr f, size_t deltaPos, bool useCache) {
    if (f) {
        fetchFileForModeInternal(
            f->sourcePath(), f->fetchMode(), deltaPos, f->sourceModTime(), f->sourceCheckSum(), useCache);
    }
}

// Get a file with the given fetch mode. We use it to fetch files appearing in the directory
// listing in the Output panel.
void OutputFileProvider::fetchFileForModeInternal(const std::string& fileName,
...
bool OutputFileProvider::isTryNoZero(const std::string& filename) const {
    if (filename.find(".0") != std::string::npos && joboutFileName() == filename && info_ && info_->isNode() &&
        info_->node() && info_->node()->node()) {

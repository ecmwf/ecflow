...
void SimpleGraphLayoutBuilder::compute_y(const std::vector<int>& nodes,
...
void SimpleGraphLayoutBuilder::printState(const std::vector<int>& nodes) {
    for (size_t i = 0; i < nodes.size(); i++) {
        ...
        std::cout << i << "[" << nodes[i] << "] "
                  << " arc=" << n->arc_ << " lev=" << n->level_ << " " << n->width_ << " " << n->height_
                  << " x=" << n->x_ << " y=" << n->y_ << " d=" << n->dummy_ << std::endl;
    }
}
...
void SimpleGraphLayoutBuilder::buildIt(bool dummy) {
    ...
    // sort by level
    std::sort(
        nodes.begin(), nodes.end(), [this](int idx1, int idx2) { return nodes_[idx1]->level_ < nodes_[idx2]->level_; });

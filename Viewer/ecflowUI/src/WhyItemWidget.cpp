...
WhyItemWidget::WhyItemWidget(QWidget* parent) : HtmlItemWidget(parent) {
    ...
    for (auto it = states.begin(); it != states.end(); ++it) {
        cssDoc += "font." + (*it)->name() + " {background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 " +
    ...
    connect(textEdit_, SIGNAL(anchorClicked(QUrl)), this, SLOT(anchorClicked(QUrl)));

    // Define the mapping for <state>stateName</state> tag replacement with
    //<font class='stateName'>stateName</font>
    for (auto& state : states) {
        stateMap_["<state>" + state->name() + "</state>"] =
            "<font class=\'" + state->name() + "\'>" + state->name() + "</font>";
    }
}

WhyItemWidget::~WhyItemWidget() = default;

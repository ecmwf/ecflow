...
TriggerItemWidget::TriggerItemWidget(QWidget* parent) : QWidget(parent) {
    ...
    connect(scanner_, SIGNAL(scanStarted()), this, SLOT(scanStarted()));

    connect(scanner_, SIGNAL(scanFinished()), this, SLOT(scanFinished()));

    connect(scanner_, SIGNAL(scanProgressed(int)), this, SLOT(scanProgressed(int)));
    ...
    connect(
        modeGroup_, SIGNAL(buttonToggled(QAbstractButton*, bool)), this, SLOT(slotChangeMode(QAbstractButton*, bool)));
#else
    connect(modeGroup_, SIGNAL(buttonToggled(int, bool)), this, SLOT(slotChangeMode(int, bool)));
#endif
    ...
    // table
    connect(triggerTable_, SIGNAL(depInfoWidgetClosureRequested()), this, SLOT(slotHandleDefInfoWidgetClosure()));

    connect(triggerTable_, SIGNAL(linkSelected(VInfo_ptr)), this, SLOT(slotLinkSelected(VInfo_ptr)));

    connect(triggerTable_,
            SIGNAL(infoPanelCommand(VInfo_ptr, QString)),
            this,
            SLOT(slotInfoPanelCommand(VInfo_ptr, QString)));

    connect(triggerTable_,
            SIGNAL(dashboardCommand(VInfo_ptr, QString)),
            this,
            SLOT(slotDashboardCommand(VInfo_ptr, QString)));

    // graph
    connect(triggerGraph_, SIGNAL(linkSelected(VInfo_ptr)), this, SLOT(slotLinkSelected(VInfo_ptr)));

    connect(triggerGraph_,
            SIGNAL(infoPanelCommand(VInfo_ptr, QString)),
            this,
            SLOT(slotInfoPanelCommand(VInfo_ptr, QString)));
...
TriggerItemWidget::~TriggerItemWidget() = default;
...
#endif
{
    ...
    if (modeGroup_->checkedId() == TableModeIndex) {
        if (triggerTable_->info() != info_ || triggerTable_->dependency() != dependency()) {

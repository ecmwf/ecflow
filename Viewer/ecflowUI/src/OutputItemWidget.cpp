...
OutputItemWidget::OutputItemWidget(QWidget* parent) : QWidget(parent) {
    ...
    // The dir listing
    connect(dirW_, SIGNAL(updateRequested()), this, SLOT(slotUpdateDirs()));

    connect(dirW_, SIGNAL(itemSelected()), this, SLOT(slotDirItemSelected()));

    connect(dirW_, SIGNAL(closedByButton()), this, SLOT(adjustShowDirTb()));

    connect(dirW_, SIGNAL(shrinkRequested()), this, SLOT(shrinkDirPanel()));
    ...
    connect(showDirTb_, SIGNAL(clicked(bool)), this, SLOT(slotShowDir(bool)));
    ...
    connect(browser_, SIGNAL(wordWrapSupportChanged(bool)), this, SLOT(slotWordWrapSupportChanged(bool)));
    ...
    connect(actionLineNumber_, SIGNAL(toggled(bool)), this, SLOT(slotLineNumber(bool)));

    connect(actionSaveFileAs_, SIGNAL(triggered()), this, SLOT(slotSaveFileAs()));

    connect(actionCopyFilePath_, SIGNAL(triggered()), this, SLOT(slotCopyPath()));
...
void OutputItemWidget::infoReady(VReply* reply) {
    ...
    if (reply->sender() == infoProvider_) {
        ...
        submittedWarning_ = false;
        // auto* op=static_cast<OutputFileProvider*>(infoProvider_);
        if (reply->fileName() == op->joboutFileName() && !op->isTryNoZero(reply->fileName()) && info_ &&
            info_->isNode() && info_->node() && info_->node()->isSubmitted()) {
            hasMessage        = true;

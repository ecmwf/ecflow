...
VariablePropDialog::VariablePropDialog(VariableModelDataHandler* data,
                                       int defineIndex,
                                       QString name,
                                       QString value,
                                       bool frozen,
    ...
    if (defineIndex_ > 0) {
        ...
        h +=
            " <br>" + EditorInfoLabel::formatKeyLabel("Inherited from: ") + EditorInfoLabel::formatNodePath(definePath);
    }
    header_->setText(h);
...
void VariablePropDialog::accept() {
    ...
    if (!data_->data(0)->hasName(name.toStdString())) {
        for (int i = 1; i < data_->count(); i++) {
            ...
            if (data_->data(i)->hasName(name.toStdString())) {
                ...
                if (QMessageBox::question(nullptr,
                                          tr("Confirm: create new variable"),
                                          tr("Variable <b>") + name + tr("</b> is originally defined in ") + type +
                                              " <b>" + path + tr("</b>. A new user variable will be created for ") +
                                              nodeType_ + " <b>" + nodeName_ +
        ...
        if (QMessageBox::question(nullptr,
                                  tr("Confirm: create new variable"),
                                  tr("You are about to create a <b>new</b> variable in ") + nodeType_ + " <b>" +
                                      nodeName_ + "</b>." + tr("<br>Do you want to proceed?"),
                                  QMessageBox::Ok | QMessageBox::Cancel,
                                  QMessageBox::Cancel) == QMessageBox::Cancel) {
            return;
        }
        ...
        if (QMessageBox::question(nullptr,
                                  QObject::tr("Confirm: change variable"),
...
void VariablePropDialog::notifyUpdated(VariableModelDataHandler*) {
    ...
    if (!st) {
        ...
        messageLabel_->showWarning("The value of variable <b>" + name + "</b> changed in " + defineNodeType_ + " <b>" +
                                   defineNodeName_ + "</b>!");
    }
    else {
        messageLabel_->hide();
...
void VariablePropDialog::writeSettings() {
    ...
    Q_ASSERT(cs);
    QSettings settings(QString::fromStdString(cs->qtSettingsFile("VariablePropDialog")), QSettings::NativeFormat);
...
void VariableAddDialog::init() {
    ...
    label_->setText(tr("Add new variable to ") + nodeType_ + " <b> " + nodeName_ + "</b>");
    //+ "<br>Path: " +
...
void VariableAddDialog::accept() {
    ...
    if (data_->data(0)->hasName(name.toStdString())) {
        ...
        if (data_->data(0)->isGenVar(name.toStdString())) {
            q = tr("Generated variable <b>") + name + tr("</b> is already defined in ") + nodeType_ + " <b>" +
                nodeName_ + "</b>" +
                tr("</b>. A new user variable will be created and the original variable will be hidden. \
                <br>Do you want to proceed?");
        }
        else {
            q = tr("User variable <b>") + name + tr("</b> is already defined in ") + nodeType_ + " <b>" + nodeName_ +
                "</b>" + tr(".<br>Do you want to overwrite it?");
        }

        if (QMessageBox::question(nullptr,
                                  tr("Confirm: overwrite variable"),
                                  q,
                                  QMessageBox::Ok | QMessageBox::Cancel,
                                  QMessageBox::Cancel) == QMessageBox::Cancel) {
            return;
    ...
    for (int i = 1; i < data_->count(); i++) {
        if (data_->data(i)->hasName(name.toStdString())) {
            ...
            q += " <b>" + name + tr("</b> is already defined in ") + nodeType + " <b>" + nodeName + "</b>" +
                 tr("</b>. A new user variable will be created for ") + nodeType_ + " <b>" + nodeName_ +
                 tr(" </b> and shadow the original one. \
                    <br>Do you want to proceed?");
...
void VariableAddDialog::slotSuspendedChanged(bool s) {
    ...
    if (s) {
        ...
        // show is still available
        if (!data_ || data_->count() == 0 || nodeName_ != QString::fromStdString(data_->data(0)->name()) ||
            nodeType_ != QString::fromStdString(data_->data(0)->type())) {
            close();
        }
    }
...
void VariableAddDialog::writeSettings() {
    SessionItem* cs = SessionHandler::instance()->current();
    Q_ASSERT(cs);
    QSettings settings(QString::fromStdString(cs->qtSettingsFile("VariableAddDialog")), QSettings::NativeFormat);
...
VariableItemWidget::VariableItemWidget(QWidget* /*parent*/) {
    ...
    connect(filterLine_, SIGNAL(textChanged(QString)), this, SLOT(slotFilterTextChanged(QString)));

    // The selection changes in the view
    connect(varView->selectionModel(),
...
VariableItemWidget::~VariableItemWidget() = default;
...
void VariableItemWidget::reload(VInfo_ptr info) {
    ...
    if (data_->count() > 0) {
        actionAdd->setText(tr("Add &new variable to ") + QString::fromStdString(data_->data(0)->name()));

        actionAdd->setToolTip(tr("Add new variable to ") + "<b>" + QString::fromStdString(data_->data(0)->name()) +
                              "</b>");
    }
...
void VariableItemWidget::editItem(const QModelIndex& index) {
    ...
    if (model_->indexToData(vIndex, block)) {
        ...
        if (model_->variable(vIndex, name, value, genVar)) {
            ...
            connect(d, SIGNAL(accepted()), this, SLOT(slotVariableEdited()));
            connect(this, SIGNAL(suspendedChanged(bool)), d, SLOT(slotSuspendedChanged(bool)));
            d->show();
        }
    }
}
...
void VariableItemWidget::addItem(const QModelIndex&) {
    ...
    if (data_->count() > 0) {
        ...
        connect(d, SIGNAL(accepted()), this, SLOT(slotVariableAdded()));
        connect(this, SIGNAL(suspendedChanged(bool)), d, SLOT(slotSuspendedChanged(bool)));
        d->show();
    }
}

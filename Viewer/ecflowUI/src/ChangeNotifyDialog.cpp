...
void ChangeNotifyDialogWidget::init(ChangeNotify* notifier) {
    ...
    // Selection
    connect(tree_, SIGNAL(clicked(const QModelIndex&)), this, SLOT(slotSelectItem(const QModelIndex&)));

    connect(tree_, SIGNAL(doubleClicked(const QModelIndex&)), this, SLOT(slotDoubleClickItem(const QModelIndex&)));
...
void ChangeNotifyDialogWidget::updateSettings() {
    ...
    QString st = "QLabel { \
                 ...
                 "; padding: 4px; border: 1px solid rgb(170,170,170);}";
...
ChangeNotifyDialog::ChangeNotifyDialog(QWidget* parent) : QDialog(parent) {
    ...
    connect(buttonGroup_,
            SIGNAL(buttonToggled(QAbstractButton*, bool)),
            this,
            SLOT(slotButtonToggled(QAbstractButton*, bool)));
#else
    connect(buttonGroup_, SIGNAL(buttonToggled(int, bool)), this, SLOT(slotButtonToggled(int, bool)));
#endif

    clearOnCloseCb_->setChecked(true);

    connect(optionsTb_, SIGNAL(clicked()), this, SLOT(slotOptions()));
...
void ChangeNotifyDialog::writeSettings() {
    ...
    Q_ASSERT(cs);
    QSettings settings(QString::fromStdString(cs->qtSettingsFile("ChangeNotifyDialog")), QSettings::NativeFormat);

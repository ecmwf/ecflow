...
LogLoadWidget::LogLoadWidget(QWidget* /*parent*/) : ui_(new Ui::LogLoadWidget) {
    ...
    connect(
        viewHandler_->data(), SIGNAL(loadProgress(size_t, size_t)), this, SLOT(slotLogLoadProgress(size_t, size_t)));
    ...
    ui_->timeLabel->setTextInteractionFlags(Qt::LinksAccessibleByMouse | Qt::TextSelectableByKeyboard |
                                            Qt::TextSelectableByMouse);

    connect(ui_->showFullTb, SIGNAL(clicked()), viewHandler_, SLOT(showFullRange()));
    ...
    connect(ui_->resCombo, SIGNAL(currentIndexChanged(int)), this, SLOT(resolutionChanged(int)));
    ...
    ui_->logView->hide();
    connect(ui_->showLogTb, SIGNAL(clicked(bool)), this, SLOT(showLogView(bool)));
    ...
    connect(viewHandler_, SIGNAL(timeRangeChanged(qint64, qint64)), logModel_, SLOT(setPeriod(qint64, qint64)));

    connect(viewHandler_, SIGNAL(timeRangeChanged(qint64, qint64)), this, SLOT(periodChanged(qint64, qint64)));

    connect(viewHandler_,
            SIGNAL(timeRangeHighlighted(qint64, qint64, qint64)),
            logModel_,
            SLOT(setHighlightPeriod(qint64, qint64, qint64)));

    connect(viewHandler_, SIGNAL(timeRangeReset()), logModel_, SLOT(resetPeriod()));

    connect(viewHandler_, SIGNAL(timeRangeReset()), this, SLOT(periodWasReset()));
...
// Initial load
void LogLoadWidget::initLoad(QString serverName,
                             QString host,
                             QString port,
                             QString logFile,
...
void LogLoadWidget::loadArchive() {
    ...
    for (int i = 0; i < archiveLogList_.items().count(); i++) {
        ...
        try {
            viewHandler_->loadMultiLogFile(archiveLogList_.items()[i].dataPath().toStdString(),
                                           suites_,
                                           i,
...
void LogLoadWidget::loadCore(QString logFile) {
    ...
    ui_->messageLabel->showInfo("Loading data from log file ... [size=" + fInfo.formatSize() + "]");
    ...
    try {
        logModel_->beginLoadFromReader();
        viewHandler_->load(logFile.toStdString(), maxReadSize_, suites_, logModel_->logData());
        logModel_->endLoadFromReader();
    }
    catch (const std::runtime_error& e) {
        logModel_->endLoadFromReader();
...
void LogLoadWidget::updateTimeLabel(QDateTime startDt, QDateTime endDt) {
    ...
    QString txt = "start: " + Viewer::formatBoldText(startDt.toString("hh:mm:ss yyyy-MM-dd"), col) +
                  " end: " + Viewer::formatBoldText(endDt.toString("hh:mm:ss yyyy-MM-dd"), col);

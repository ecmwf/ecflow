...
void TextFilterWidget::setExternalButtons(QToolButton* statusTb, QToolButton* optionTb) {
    ...
    connect(statusTb_, SIGNAL(toggled(bool)), this, SLOT(slotStatusTb(bool)));

    connect(optionTb_, SIGNAL(clicked()), this, SLOT(slotOptionTb()));
}
...
void TextFilterWidget::addMenuSection(QMenu* menu,
...
void TextFilterWidget::adjustToolTip() {
    ...
    if (status_ == FoundStatus || status_ == NotFoundStatus) {
        ...
        filterDesc += tr("Current filter:") + "<br><b>&nbsp;regexp:</b> " + filterText() + "<br><b>&nbsp;mode: </b>" +
                      ...
                      +(isCaseSensitive() ? "case sensitive" : "case insensitive") + "<br><br>";
    }

    switch (status_) {
        case EditStatus:
            statusTb_->setToolTip(
                tr(isVisible() ? "Toggle to <b>hide</b> text filter bar. The filter remains active (if defined)."
                               : "Toggle to <b>show</b> text filter bar"));
            break;
        case FoundStatus:
            statusTb_->setToolTip(filterDesc + tr("There ") + Viewer::formatText("are lines", QColor(100, 220, 120)) +
                                  tr(" matching the filter in the output file"));
            break;
        case NotFoundStatus:
            statusTb_->setToolTip(filterDesc + tr("There ") + Viewer::formatText("are no lines", QColor(255, 95, 95)) +
                                  tr(" matching the filter in the output file"));
            break;
        default:
            break;
...
void TextFilterWidget::addCurrentToLatest() {
    TextFilterHandler::Instance()->addLatest(filterText().toStdString(), isMatched(), isCaseSensitive(), true);
}

bool TextFilterWidget::isCurrentSaved() const {
    return TextFilterHandler::Instance()->contains(filterText().toStdString(), isMatched(), isCaseSensitive());
}

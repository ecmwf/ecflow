...
void ServerRefreshInfoWidget::drawButton(QPainter* painter) {
    ...
    if (server_ && isEnabled()) {
        // blink
        if (inRefresh_ &&
            ((mode_ == NormalMode && period_ >= noBlinkLimit_) || userInitiatedRefresh_ || mode_ == ManualMode)) {
    ...
    QPixmap pix =
        icon_->pixmap(QSize(r2.width(), r2.width()), refreshAction_->isEnabled() ? QIcon::Normal : QIcon::Disabled);
    ...
    */
    if (true) {
        ...
        if (hasInfo_) {
            ...
            {
                txt+="<br>--------------------------------------------<br>";
                txt+="When <b>drift</b> is enabled the server refresh period is increased at every automatic refresh
            until \ the maximum period is reached. The drift is reset to zero when the user interacts with the server.";

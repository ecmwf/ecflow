...
ScriptProvider::ScriptProvider(InfoPresenter* owner) : InfoProvider(owner)
{
...
void ScriptProvider::visit(VInfoNode* info)
{
	...
	task_=VTask::create(VTask::ScriptTask,info->node(),this);
...
void  ScriptProvider::taskChanged(VTask_ptr task)
{
	if(task_ != task)
		return;
